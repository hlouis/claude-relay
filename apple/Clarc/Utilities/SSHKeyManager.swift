...
/// Manages an ed25519 SSH key dedicated to Clarc for GitHub access.
///
...
actor SSHKeyManager {
    ...
    /// Clarc key, but only if such an entry is not already present.
    func configureSSHConfig() async throws {
        ...
        logger.info("Added Clarc SSH config entry.")
    }
