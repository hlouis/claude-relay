...
import { initRewind, setRewindMode, showRewindModal, hideRewindLoading, clearPendingRewindUuid, addRewindButton, getRewindUserText, clearRewindUserText } from './modules/rewind.js';
import { initNotifications, showDoneNotification, playDoneSound, isNotifAlertEnabled, isNotifSoundEnabled } from './modules/notifications.js';
import { initInput, clearPendingImages, handleInputSync, autoResize, builtinCommands, sendMessage, handleAtFsListResult } from './modules/input.js';
...
import { initCodexSkills, setCodexSkillsCapability, handleCodexSkillsMessage, getCodexSkillsState, maybeShowInlineMenu, hideInlineMenu, isInlineMenuVisible, moveInlineSelection, commitInlineSelection } from './modules/codex-skills.js';
...
// --- DOM refs ---
  ...
  var pendingResume = false; // true when reconnecting with incremental resume
  var disconnectNotifTimer = null;
  var disconnectNotifShown = false;
  var overlayDelayTimer = null;
  var activityEl = null;
  ...
  var sessionDrafts = {};
  // Per-session message cache for incremental rendering on session/project switch
  // Key: "slug:sessionId", Value: { lastSeq, messages[] }
  var messageCache = {};
  var MESSAGE_CACHE_SIZE = 200;
  ...
  var lastSeq = -1; // highest seq received, for incremental reconnect
  ...
  // --- Iter 5b: Codex thread fork ---
  // The button is hidden by default and revealed by the info handler when
  // info.capabilities.threadFork is true. Click → confirm → emit
  // fork_thread; the server takes over from there (creates a new session,
  // broadcasts the list, the sidebar self-updates).
  var headerForkBtn = $("header-fork-btn");
  if (headerForkBtn) {
    headerForkBtn.addEventListener("click", function () {
      if (!activeSessionId || !ws || ws.readyState !== 1) return;
      // Use the existing custom confirm modal — CLAUDE.md forbids native
      // confirm()/alert(). destructive=false picks the "ok" button style;
      // okLabel matches the action verb.
      showConfirm(
        "Fork this conversation? The current thread will remain accessible from the session list.",
        function () {
  ...
  var sidebarCtx = {
    ...
    // Iter 4: sidebar reads this to decide which session rows get the
    // "diverges-from-default" backend badge. Live getter so a
    // project_backend_changed broadcast immediately retargets the badges
    // (the next session_list re-render reads the new value).
    get currentDefaultBackend() { return currentProjectDefaultBackend; },
  ...
  function setStatus(status) {
    ...
    if (status === "connected") {
      ...
      if (!overlayDelayTimer) {
        overlayDelayTimer = setTimeout(function () {
  ...
  var isOsUsers = false;
  // Iter 3 protocol split:
  //   currentProjectBackend = active session's frozen backend (drives UI:
  //     body.backend-codex, fork button, skill picker, settings drawer).
  //   currentProjectDefaultBackend = project's "new session" default
  //     (used by iter-4 settings UI when offering to flip the default).
  // In single-backend projects these are equal at all times. They diverge
  // only when a project hosts sessions of mixed backends and the user
  // navigates between them.
  //
  // The variable name is preserved for compat with module-exposed getters
  // and the many internal reads (chip rendering, effort levels, etc.) —
  // they all want "the active session's backend" which is now what this
  // tracks.
  var currentProjectBackend = "claude";
  var currentProjectDefaultBackend = "claude";
  // Capabilities surface for the ACTIVE SESSION (iter 3). Re-derived on
  // every session_switched so a Claude session in a Codex project gets
  // Claude's settings list + threadFork=false, and vice versa. Default
  // mirrors Claude's historical full set so any pre-capability path works.
  var currentProjectCapabilities = { settings: ["model", "permissionMode", "effort", "betas", "thinking"] };
  // Codex-side permission model state (iter 3 step 4). Mirrors codex-backend's
  // `desiredSandbox` / `desiredApprovalPolicy` and is updated via the
  // `codex_config` echo. Defaults match codex-backend's defaults.
  var currentCodexSandbox = "workspace-write";
  var currentCodexApproval = "on-request";

  // Iter 3: applies the active session's backend identity to all UI
  // affordances that depend on it. Called from BOTH the `info` handler
  // (initial paint + reconnect) AND `session_switched` (when the user
  // navigates between sessions of different backends in one project).
  //
  // Idempotent: calling with the same args twice produces the same DOM.
  // Tolerant of missing `capabilities`: legacy servers (or session_switched
  // frames before iter 3 was fully wired) may omit them, in which case we
  // keep whatever caps were last set by `info`. This is correct because
  // capabilities are a project-default-backend property in the legacy
  // protocol — the worst case is the topbar Fork button briefly showing
  // for a Claude session inside a Codex project, which clears the moment
  // the next `info` arrives.
  function applyActiveSessionBackend(backendName, capabilities) {
    if (backendName) currentProjectBackend = backendName;
    if (capabilities && Array.isArray(capabilities.settings)) {
      currentProjectCapabilities = capabilities;
    }
    if (document.body) {
      document.body.classList.toggle("backend-codex", currentProjectBackend === "codex");
    }
    updateBackendChip();
    // Topbar Fork button (Codex-only HEAD fork). Re-evaluated on every
    // call so a navigation between Claude/Codex sessions flips it.
    var forkBtn = $("header-fork-btn");
    if (forkBtn) {
      var canFork = !!(currentProjectCapabilities && currentProjectCapabilities.threadFork);
      forkBtn.hidden = !canFork;
    }
    // Codex skills picker capability gate (topbar btn + popover panel).
    setCodexSkillsCapability(!!(currentProjectCapabilities && currentProjectCapabilities.codexSkills));
  }

  // Codex permission model option enums (kept in sync with codex-backend.js
  // whitelists in setSandbox / setApprovalPolicy).
  var CODEX_SANDBOX_OPTIONS = [
    { value: "read-only", label: "Read", desc: "Block all writes." },
    { value: "workspace-write", label: "Workspace", desc: "Allow writes inside the project." },
    { value: "danger-full-access", label: "Full", desc: "No sandbox. Use with caution." },
  ];
  var CODEX_APPROVAL_OPTIONS = [
    { value: "untrusted", label: "Untrusted", desc: "Ask before any tool use." },
    { value: "on-failure", label: "On-failure", desc: "Auto-approve until something fails." },
    { value: "on-request", label: "On-request", desc: "Ask only when the agent escalates." },
    { value: "never", label: "Never", desc: "Auto-approve everything." },
  ];
  // Codex's reasoning effort enum is a subset of Claude's. Bound at popup
  // render time so the bar doesn't show levels the backend would silently
  // drop.
  var CODEX_EFFORT_LEVELS = ["low", "medium", "high"];
  ...
  function cycleMode() {
    ...
    if (skipPermsEnabled) options.push(MODE_FULL_AUTO);
    var idx = 0;
    for (var i = 0; i < options.length; i++) {
      if (options[i].value === currentMode) { idx = i; break; }
    }
    var next = options[(idx + 1) % options.length];
    if (ws && ws.readyState === 1) {
      ws.send(JSON.stringify({ type: "set_permission_mode", mode: next.value }));
    }
  }
  ...
  // Topbar chip that identifies the project's agent backend. Iter 1 only
  // renders for Codex projects — Claude is the implicit default and gets
  // no chip (matches the rest of Clay where "no badge" means Claude).
  function updateBackendChip() {
    var chip = document.getElementById("header-backend-chip");
    if (!chip) return;
    if (currentProjectBackend === "codex") {
      ...
      if (currentModel) {
        ...
        sep.className = "header-backend-chip-model";
        sep.textContent = "· " + currentModel;
  ...
  function updateConfigChip() {
    ...
    var parts = [modelDisplayName(currentModel, currentModels) || (currentProjectBackend === "codex" ? "codex" : "")];
    if (currentProjectBackend === "codex") {
      // Codex chip label: model \u00b7 sandbox \u00b7 effort. Sandbox is the
      // user-visible permission knob; mode/thinking/beta don't apply.
      parts.push(currentCodexSandbox);
    } else {
    ...
    rebuildModelList();
    rebuildModeList();
    rebuildCodexPermissionsSections();
    rebuildEffortBar();
  ...
  function getModelEffortLevels() {
    // Codex's reasoning effort enum is a fixed subset (no `max`). Short-
    // circuit before consulting currentModels, which for Codex is empty
    // anyway.
    if (currentProjectBackend === "codex") return CODEX_EFFORT_LEVELS;
  ...
  // Iter 3 step 4: render Codex's sandbox + approval segmented controls
  // into the popup. CSS keeps the sections hidden on Claude projects so we
  // can call this unconditionally; the sandbox/approval bars are a no-op
  // for Claude users.
  function rebuildCodexPermissionsSections() {
    if (currentProjectBackend !== "codex") return;
    buildCodexSegmented("config-codex-sandbox-bar", CODEX_SANDBOX_OPTIONS, currentCodexSandbox, function (v) {
      currentCodexSandbox = v;
      if (ws && ws.readyState === 1) {
        ws.send(JSON.stringify({ type: "set_codex_sandbox", sandbox: v }));
      }
      updateConfigChip();
    });
    buildCodexSegmented("config-codex-approval-bar", CODEX_APPROVAL_OPTIONS, currentCodexApproval, function (v) {
      currentCodexApproval = v;
      if (ws && ws.readyState === 1) {
        ws.send(JSON.stringify({ type: "set_codex_approval_policy", approvalPolicy: v }));
      }
      updateConfigChip();
    });
  }

  function buildCodexSegmented(containerId, options, currentValue, onPick) {
    ...
    for (var i = 0; i < options.length; i++) {
      (function (opt) {
        var btn = document.createElement("button");
        btn.className = "config-segment-btn" + (opt.value === currentValue ? " active" : "");
        btn.textContent = opt.label;
        btn.title = opt.desc || "";
        btn.dataset.value = opt.value;
        btn.addEventListener("click", function () {
          var btns = bar.querySelectorAll(".config-segment-btn");
  ...
  // --- DOM: Messages ---
  function addUserMessage(text, images, pastes, clientMsgId) {
  ...
  // Codex guidance card. Unlike the Claude flow, we never auto-spawn a
  // terminal or poll for auth state — the user runs `codex login` in their
  // own shell and clicks Retry when ready. This keeps Clay out of the
  // OAuth path (the entire point of Iter 1's auth strategy).
  function addCodexAuthGuidanceCard(msg) {
    var div = document.createElement("div");
    div.className = "auth-required-msg codex-auth-card";

    var header = document.createElement("div");
    header.className = "auth-required-header";
    header.textContent = "Codex is not logged in";
    div.appendChild(header);

    var hint = document.createElement("div");
    hint.className = "auth-required-hint";
    var detail = msg.text || "Run `codex login` in a terminal on this machine, then click Retry below.";
    // Strip the "codex_auth: " prefix the backend prepends to its error.
    detail = detail.replace(/^codex_auth:\s*/, "");
    hint.textContent = detail;
    div.appendChild(hint);

    var cmdRow = document.createElement("div");
    cmdRow.className = "codex-auth-cmd-row";
    var cmd = document.createElement("code");
    cmd.className = "codex-auth-cmd";
    cmd.textContent = "codex login";
    cmdRow.appendChild(cmd);
    var copyBtn = document.createElement("button");
    copyBtn.className = "codex-auth-copy-btn";
    copyBtn.type = "button";
    ...
    copyBtn.addEventListener("click", function () {
      ...
      try {
        if (navigator.clipboard && navigator.clipboard.writeText) {
          navigator.clipboard.writeText("codex login").then(done, done);
        } else {
          // Fallback for older browsers / non-secure contexts.
          var ta = document.createElement("textarea");
          ta.value = "codex login";
    ...
    retryBtn.className = "codex-auth-retry-btn";
    retryBtn.type = "button";
    retryBtn.textContent = "Retry";
    retryBtn.addEventListener("click", function () {
      // No polling — the user pressed Retry, so we just clear the card,
      // restore the composer, and let the next message attempt re-trigger
      // the backend's auth pre-flight. If auth still fails, the same
      // guidance card lands again.
      div.remove();
      var inputArea = document.getElementById("input-area");
      if (inputArea) inputArea.classList.remove("hidden");
      if (inputEl) {
        inputEl.disabled = false;
        inputEl.placeholder = "Message Codex...";
        try { inputEl.focus(); } catch (e) {}
      }
      if (sendBtn) sendBtn.disabled = false;
    });
    actions.appendChild(retryBtn);
    var retryHint = document.createElement("span");
    retryHint.className = "codex-auth-retry-hint";
    retryHint.textContent = "After running the command above";
    actions.appendChild(retryHint);
    div.appendChild(actions);

    addToMessages(div);
    scrollToBottom();

    // Hide the composer until the user explicitly retries — keeps them
    // from sending more messages that will all fail the same way.
    var inputArea = document.getElementById("input-area");
    if (inputArea) inputArea.classList.add("hidden");
  }

  // Iter 4: codex_unavailable card. The backend emits this whenever the
  // codex app-server cannot proceed (subprocess crash, binary uninstalled,
  // future: auth lost / version incompatible). One renderer, kind-driven
  // copy + recovery action — no special-case branches in the WS routing.
  function addCodexUnavailableCard(msg) {
    var kind = msg.kind || "crashed";
    var div = document.createElement("div");
    div.className = "codex-unavailable-card codex-unavailable-" + kind;
    div.setAttribute("data-kind", kind);

    var titleByKind = {
      crashed: "Codex app-server stopped",
      binary_missing: "Codex binary not found",
      auth_lost: "Codex authentication expired",
      version_incompatible: "Codex version is incompatible",
    };
    var hintByKind = {
      crashed: "The Codex subprocess exited unexpectedly. Click Retry to restart it.",
      binary_missing: "The `codex` command is not on this machine's PATH. Install or reinstall the CLI, then click Retry.",
      auth_lost: "Run `codex login` in a terminal, then click Retry.",
      version_incompatible: "The installed Codex CLI is too old for this Clay version. Run `codex update` (or reinstall) and click Retry.",
    };

    var header = document.createElement("div");
    header.className = "codex-unavailable-header";
    header.textContent = titleByKind[kind] || titleByKind.crashed;
    div.appendChild(header);

    var detail = document.createElement("div");
    detail.className = "codex-unavailable-detail";
    detail.textContent = msg.message || hintByKind[kind] || "";
    div.appendChild(detail);

    if (hintByKind[kind] && (!msg.message || msg.message.indexOf(hintByKind[kind]) === -1)) {
      var hint = document.createElement("div");
      hint.className = "codex-unavailable-hint";
      hint.textContent = hintByKind[kind];
      div.appendChild(hint);
    }
    ...
    retryBtn.textContent = "Retry";
    retryBtn.addEventListener("click", function () {
      try { if (ws && ws.readyState === 1) ws.send(JSON.stringify({ type: "codex_retry" })); } catch (e) {}
      retryBtn.disabled = true;
      retryBtn.textContent = "Retrying...";
      // The card is dismissed on the next codex_unavailable (failure) or
      // model_info (success) — see the WS handler. Don't remove it here.
    });
    actions.appendChild(retryBtn);
    ...
    logsBtn.addEventListener("click", function () {
      if (logsPane.style.display === "none") {
        ...
        // Always re-fetch so the user gets the freshest snapshot, even if
        // the card has been on screen a while.
        try { if (ws && ws.readyState === 1) ws.send(JSON.stringify({ type: "codex_logs_request" })); } catch (e) {}
        // Render the inline stderrTail immediately so the user sees
        // something while waiting for codex_logs_response.
        if (msg.stderrTail) {
          renderCodexLogsInto(logsPane, { stderrTail: msg.stderrTail, logFile: null, logTail: "" });
        } else {
          logsPane.textContent = "Loading logs...";
        }
        // Stash the pane so the WS handler can update it.
        window.__codexLogsPane = logsPane;
      } else {
        logsPane.style.display = "none";
        logsBtn.textContent = "View Logs";
      }
    ...
    addToMessages(div);
    scrollToBottom();
    // Stash the card so model_info / a fresh codex_unavailable can manage it.
    window.__codexUnavailableCard = div;
  }

  function renderCodexLogsInto(pane, payload) {
    pane.textContent = "";
    var stderrSection = document.createElement("div");
    stderrSection.className = "codex-unavailable-logs-section";
    var stderrTitle = document.createElement("div");
    stderrTitle.className = "codex-unavailable-logs-title";
    stderrTitle.textContent = "stderr (last 8 KB)";
    stderrSection.appendChild(stderrTitle);
    var stderrPre = document.createElement("pre");
    stderrPre.className = "codex-unavailable-logs-pre";
    stderrPre.textContent = payload.stderrTail || "(empty)";
    stderrSection.appendChild(stderrPre);
    pane.appendChild(stderrSection);

    if (payload.logFile || payload.logTail) {
      var logSection = document.createElement("div");
      logSection.className = "codex-unavailable-logs-section";
      var logTitle = document.createElement("div");
      logTitle.className = "codex-unavailable-logs-title";
      logTitle.textContent = payload.logFile
        ? ("~/.codex/log/" + payload.logFile.split("/").pop())
        : "~/.codex/log";
      logSection.appendChild(logTitle);
      var logPre = document.createElement("pre");
      logPre.className = "codex-unavailable-logs-pre";
      logPre.textContent = payload.logTail || "(empty)";
  ...
  function addAuthRequiredMessage(msg) {
    // Codex projects render a dedicated guidance card instead of the
    // Claude auto-terminal flow. They share the .auth-required-msg base
    // styles but get an extra .codex-auth-card modifier for theming.
    if (msg.source === "codex") {
      addCodexAuthGuidanceCard(msg);
      return;
    }
  ...
  // Notify server when tab becomes visible so cross-project unread badges clear
  document.addEventListener("visibilitychange", function () {
    if (!document.hidden && ws && ws.readyState === 1) {
      ws.send(JSON.stringify({ type: "tab_visible" }));
    }
  });
  ...
  function connect() {
    ...
    // Incremental reconnect: send lastSeq so server only sends new messages
    // Only for same-project reconnect (cache-based session switch uses switch_session msg)
    pendingResume = false;
    var cacheKey = activeSessionId ? (currentSlug || "_") + ":" + activeSessionId : null;
    var cached = cacheKey ? messageCache[cacheKey] : null;
    if (cached && cached.lastSeq > 0 && activeSessionId) {
      wsUrl += (wsUrl.indexOf("?") === -1 ? "?" : "&") + "resumeSession=" + activeSessionId + "&lastSeq=" + cached.lastSeq;
      pendingResume = true;
    }
    ...
    ws.onopen = function () {
      ...
      // Wrap ws.send to blink LED and inject lastSeq for switch_session
      var _origSend = ws.send.bind(ws);
      ws.send = function (data) {
        blinkIO();
        // Auto-attach lastSeq from cache for switch_session messages
        if (typeof data === "string" && data.indexOf("switch_session") !== -1) {
          try {
            var parsed = JSON.parse(data);
            if (parsed.type === "switch_session" && parsed.id && typeof parsed.lastSeq !== "number") {
              var ck = (currentSlug || "_") + ":" + parsed.id;
              var mc = messageCache[ck];
              if (mc && mc.lastSeq > 0) {
                parsed.lastSeq = mc.lastSeq;
  ...
  function messageCacheKey() {
    return (currentSlug || "_") + ":" + activeSessionId;
  }

  function cacheMessage(msg) {
    if (typeof msg.seq !== "number" || !activeSessionId) return;
    var key = messageCacheKey();
    if (!messageCache[key]) messageCache[key] = { lastSeq: -1, messages: [] };
    var mc = messageCache[key];
    if (msg.seq + 1 > mc.lastSeq) {
      mc.messages.push(msg);
      mc.lastSeq = msg.seq + 1;
      if (mc.messages.length > MESSAGE_CACHE_SIZE) {
        mc.messages.splice(0, mc.messages.length - MESSAGE_CACHE_SIZE);
      }
    }
  }

  function processMessage(msg) {
      // Track highest seq and cache for incremental reconnect
      if (typeof msg.seq === "number" && msg.seq >= lastSeq) {
        lastSeq = msg.seq + 1; // next expected seq
      }
      cacheMessage(msg);
      switch (msg.type) {
        case "history_meta":
          if (msg.resumed) {
            // Incremental reconnect — don't touch historyFrom (DOM already has older messages)
            historyTotal = msg.total;
            pendingResume = false;
          } else {
            // Full replay — if we skipped reset during session_switched (pendingResume),
            // do it now since server rejected the resume (e.g. rewind happened)
            var wasPendingResume = pendingResume;
            if (pendingResume) {
              pendingResume = false;
              lastSeq = -1;
              // Invalidate stale cache (e.g. rewind happened)
              var staleKey = messageCacheKey();
              if (staleKey) delete messageCache[staleKey];
              clearRemoteCursors();
              resetClientState();
            }
        ...
        case "info":
          ...
          // Iter 3 protocol split:
          //   msg.defaultBackend = project's "new session" default.
          //   msg.backend        = active session's frozen backend (drives UI).
          // Older servers send only `backend` — fall back to it for default.
          if (msg.defaultBackend) {
            currentProjectDefaultBackend = msg.defaultBackend;
          } else if (msg.backend) {
            currentProjectDefaultBackend = msg.backend;
          }
          // setTitle's broadcast info OMITS `backend` and `capabilities` so
          // a client whose active session differs from the project default
          // doesn't have its UI clobbered. Only update active-session UI
          // when the server actually sent the active-session backend.
          if (msg.backend) {
            applyActiveSessionBackend(msg.backend, msg.capabilities);
          }
          if (msg.codex) {
            codexAvailable = !!msg.codex.binAvailable;
            if (!codexAvailable) {
              codexUnavailableReason = "Install `codex` CLI to enable this backend (https://developers.openai.com/codex).";
            } else if (!msg.codex.authOk) {
              // Binary present but not logged in: keep the radio enabled.
              // Submitting will trigger the in-project guidance flow.
              codexUnavailableReason = "";
            }
            refreshCodexBackendBtn();
          }
        ...
        case "update_available":
          ...
          if (updatePillWrap && updateVersion && msg.version) {
            ...
            if (updManualCmd) {
              ...
              updManualCmd.textContent = "npx @hlouis/clay@" + updTag;
            }
            refreshIcons();
          }
        ...
        case "model_info":
          ...
          // Codex backend ships its own model identifier piggybacked on
          // model_info; refresh the topbar chip so it picks up the latest.
          updateBackendChip();
          // Iter 4: a successful Codex warmup/retry clears any visible
          // codex_unavailable card. Only treat backend === "codex" as the
          // dismissal signal so Claude's model_info (which fires on every
          // project load) doesn't accidentally swallow the card.
          if (msg.backend === "codex" && window.__codexUnavailableCard) {
            try {
              if (window.__codexUnavailableCard.parentNode) {
                window.__codexUnavailableCard.parentNode.removeChild(window.__codexUnavailableCard);
              }
            } catch (e) {}
            window.__codexUnavailableCard = null;
            window.__codexLogsPane = null;
          }
          break;

        case "codex_unavailable":
          // Iter 4: Codex client cannot proceed. Render a kind-driven card.
          // If a previous card is still visible, replace it (its Retry was
          // probably what triggered this new failure).
          try {
            if (window.__codexUnavailableCard && window.__codexUnavailableCard.parentNode) {
              window.__codexUnavailableCard.parentNode.removeChild(window.__codexUnavailableCard);
            }
          } catch (e) {}
          window.__codexUnavailableCard = null;
          window.__codexLogsPane = null;
          addCodexUnavailableCard(msg);
          break;

        case "codex_fork_error":
          // Iter 5b: surface fork failures inline so the user sees what
          // went wrong without burying it in a console. We deliberately
          // do NOT reuse the codex_unavailable card — fork-failure is a
          // one-shot operational error, not a "client cannot proceed"
          // state.
          try {
            var ferr = document.createElement("div");
            ferr.className = "codex-fork-error";
            ferr.textContent = "Fork failed: " + (msg.reason || "unknown reason");
            ...
            fdismiss.title = "Dismiss";
            fdismiss.addEventListener("click", function () {
              try { ferr.remove(); } catch (e) {}
            });
            ferr.appendChild(fdismiss);
            messagesEl.appendChild(ferr);
            messagesEl.scrollTop = messagesEl.scrollHeight;
          } catch (e) { console.error("[codex-fork-error] render failed:", e); }
          break;

        case "codex_logs_response":
          // Iter 4: refresh the inline logs pane if one is open.
          if (window.__codexLogsPane) {
            try { renderCodexLogsInto(window.__codexLogsPane, msg); } catch (e) {}
          }
          break;

        case "codex_config":
          // Iter 3 step 3/4: codex-backend echoes sandbox/approvalPolicy so
          // the settings panel and config chip popup show the active
          // selection. Mirror to local state + global stash (the latter is
          // read by project-settings.js's populateDefaultsCodex).
          //
          // Iter 4 follow-up: the first-connection echo also carries
          // model + effort so the chip's first paint reflects daemon.json
          // instead of frontend hard-coded defaults. We only update local
          // state when the field is explicitly populated — the runtime
          // setSandbox/setApprovalPolicy echoes omit model/effort and
          // must not stomp on whatever model_info/config_state set.
          if (typeof msg.sandbox === "string") currentCodexSandbox = msg.sandbox;
          if (typeof msg.approvalPolicy === "string") currentCodexApproval = msg.approvalPolicy;
          if (typeof msg.model === "string" && msg.model) currentModel = msg.model;
          if (typeof msg.effort === "string" && msg.effort) currentEffort = msg.effort;
          window.__codexConfig = { sandbox: currentCodexSandbox, approvalPolicy: currentCodexApproval };
          updateConfigChip();
          break;

        case "codex_skills":
          // Iter 6a: replace the entire skills cache. The frame is the
          // authoritative snapshot — codex backend rebroadcasts on every
          // skills/changed notification + on user-initiated refresh.
          handleCodexSkillsMessage(msg);
          break;

        case "project_backend_changed":
          // Iter 4: project's default backend changed. This affects only
          // future sessions — the active session's UI must NOT change.
          // We update the cached default and re-paint the settings panel
          // backend toggle if it happens to be open. No body class flip.
          if (msg.defaultBackend) {
            currentProjectDefaultBackend = msg.defaultBackend;
            // If the project settings panel is open on the Defaults tab,
            // re-render the backend card so the active button reflects
            // the new value (also useful when another browser tab made
            // the change).
            var psPanel = document.getElementById("project-settings");
            if (psPanel && !psPanel.classList.contains("hidden")) {
              if (window.__projectSettingsRefreshDefaults) {
                try { window.__projectSettingsRefreshDefaults(); } catch (e) {}
              }
            }
          }
          break;
        ...
        case "session_switched":
          hideHomeHub();
          // Iter 3: re-target backend-dependent UI to the incoming session's
          // frozen backend identity BEFORE we touch DOM/state for the
          // session itself. body.backend-codex flipping early means the
          // CSS rules in codex.css apply during the replay, avoiding a
          // flash of the wrong backend's chrome. msg.backend / capabilities
          // may be absent on pre-iter-3 servers — applyActiveSessionBackend
          // tolerates that (keeps previous values).
          if (msg.backend) {
            applyActiveSessionBackend(msg.backend, msg.capabilities);
          }
          ...
          // 1. WS reconnect to same session (pendingResume already set by connect())
          // 2. Session switch with cached messages → re-render from cache
          // 3. No cache → full reset, wait for server replay
          var isResume = pendingResume && msg.id === activeSessionId;
          if (!isResume) {
            var switchCacheKey = (currentSlug || "_") + ":" + msg.id;
            var switchCache = messageCache[switchCacheKey];
            if (switchCache && switchCache.lastSeq > 0) {
              // Have cached messages — re-render from cache, then let server send incremental
              pendingResume = true;
              lastSeq = switchCache.lastSeq;
              activeSessionId = msg.id;
              cliSessionId = msg.cliSessionId || null;
              clearRemoteCursors();
              resetClientState();
              // Replay cached messages to rebuild DOM instantly
              replayingHistory = true;
              historyFrom = switchCache.messages.length > 0 ? switchCache.messages[0].seq : 0;
              historyTotal = switchCache.lastSeq;
              for (var ci = 0; ci < switchCache.messages.length; ci++) {
                processMessage(switchCache.messages[ci]);
              }
              replayingHistory = false;
              markAllToolsDone();
              finalizeAssistantBlock();
          ...
          inputEl.value = draft;
          autoResize();
          if (!("ontouchstart" in window) && !processing) {
            inputEl.focus();
          }
          break;
        ...
        case "user_message":
          // Dedupe optimistically-rendered bubble from the sender
          if (msg.clientMsgId) {
            var existing = messagesEl.querySelector('.msg-user[data-client-msg-id="' + msg.clientMsgId + '"]');
            if (existing) break;
          }
          resetThinkingGroup();
          if (msg.planContent) {
            ...
            addUserMessage("Execute the following plan. Do NOT re-enter plan mode — just implement it step by step.", msg.images || null, msg.pastes || null, msg.clientMsgId || null);
          } else {
            addUserMessage(msg.text, msg.images || null, msg.pastes || null, msg.clientMsgId || null);
          }
          break;
        ...
        case "permission_request":
          renderPermissionRequest(msg.requestId, msg.toolName, msg.toolInput, msg.decisionReason, msg.source);
          startUrgentBlink();
          break;
        ...
        case "rewind_complete":
          ...
          var savedUserText = getRewindUserText();
          if (savedUserText) {
            inputEl.value = savedUserText;
            clearRewindUserText();
          }
          break;
        ...
        case "fs_list_result":
          if (msg.source === "at-complete") {
            handleAtFsListResult(msg);
          } else {
            handleFsList(msg);
          }
          break;
  ...
  initInput({
    ...
    // Iter 4 follow-up: input.js filters Claude-only slash commands
    // (e.g. /rewind) when the active session is Codex. Live getter so a
    // session_switched between Claude and Codex inside one project takes
    // effect immediately on the next slash-menu open.
    currentBackend: function () { return currentProjectBackend; },
  ...
  initProjectSettings({
    ...
    get currentThinking() { return currentThinking; },
    get currentThinkingBudget() { return currentThinkingBudget; },
    get currentBackend() { return currentProjectBackend; },
    // Iter 4: project's default-backend-for-new-sessions, distinct from
    // currentBackend (= active session's frozen backend). Settings panel
    // reads this to highlight the right segmented button.
    get currentDefaultBackend() { return currentProjectDefaultBackend; },
    get currentCapabilities() { return currentProjectCapabilities; },
    get currentCodexConfig() { return window.__codexConfig || null; },
  }, getEmojiCategories());
  ...
  // --- Codex skills (Iter 6a) ---
  // Sibling of Claude's Skills system, kept distinct because the data
  // source (codex skills/list) and invocation syntax (`$<name>`) differ.
  // Capability gate happens on the info handler below.
  initCodexSkills({
    inputEl: inputEl,
    autoResize: autoResize,
    sendInputSync: function () { /* input.js owns the broadcast; we just trigger an input event */ },
    requestCodexSkills: function (forceReload) {
      if (!ws || ws.readyState !== 1) return;
      ws.send(JSON.stringify({ type: "request_codex_skills", forceReload: !!forceReload }));
    },
  });
  ...
  var addProjectBackendBtns = addProjectModal.querySelectorAll(".add-project-backend-btn");
  var addProjectDebounce = null;
  var addProjectActiveIdx = -1;
  var addProjectMode = "existing";
  var addProjectBackend = "claude";

  // Codex availability is reported by the server in the initial config
  // payload (see handleConfig below). Until we hear otherwise, the Codex
  // radio is disabled with a tooltip pointing the user at install instructions.
  var codexAvailable = false;
  var codexUnavailableReason = "Install codex-cli to enable this backend.";

  function refreshCodexBackendBtn() {
    for (var bi = 0; bi < addProjectBackendBtns.length; bi++) {
      var b = addProjectBackendBtns[bi];
      if (b.dataset.backend !== "codex") continue;
      if (codexAvailable) {
        b.disabled = false;
        ...
        b.disabled = true;
        b.title = codexUnavailableReason;
        b.dataset.tooltip = codexUnavailableReason;
        // If codex was previously selected, fall back to claude.
        if (addProjectBackend === "codex") setAddProjectBackend("claude");
      }
    }
  }

  function setAddProjectBackend(backend) {
    addProjectBackend = backend === "codex" ? "codex" : "claude";
    for (var bi = 0; bi < addProjectBackendBtns.length; bi++) {
      var b = addProjectBackendBtns[bi];
      var on = b.dataset.backend === addProjectBackend;
      b.classList.toggle("active", on);
      b.setAttribute("aria-checked", on ? "true" : "false");
    }
  }

  for (var bbi = 0; bbi < addProjectBackendBtns.length; bbi++) {
    addProjectBackendBtns[bbi].addEventListener("click", function () {
      if (this.disabled) return;
      setAddProjectBackend(this.dataset.backend);
    });
  }
  ...
  function openAddProjectModal() {
    ...
    // Reset backend selector each open. The Codex radio reflects the
    // server-reported availability (see handleConfig).
    setAddProjectBackend("claude");
    refreshCodexBackendBtn();
