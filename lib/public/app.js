...
// --- DOM refs ---
  ...
  // Capabilities surface declared by the backend (Iter 3 step 2). Currently
  // only `settings: [...]` is populated; the settings drawer (step 3 onward)
  // reads it to decide which controls to render. Default mirrors Claude's
  // historical full set so any pre-capability code path keeps working.
  ...
  function processMessage(msg) {
      ...
      switch (msg.type) {
        ...
        case "info":
          ...
          if (msg.backend) currentProjectBackend = msg.backend;
          if (msg.capabilities && Array.isArray(msg.capabilities.settings)) {
            currentProjectCapabilities = msg.capabilities;
          }
          // Toggle a body class so backend-specific CSS (lib/public/css/codex.css)
          // can hide affordances that don't apply to Codex projects yet
          // (Rewind, Skills, Ralph Loop, slash commands).
          if (document.body) {
          ...
          updateBackendChip();
          // Iter 5b: Codex projects expose a HEAD-only thread fork. Show
          // the topbar Fork button only when the backend explicitly
          // advertises the capability (Claude does not).
          var forkBtn = $("header-fork-btn");
          if (forkBtn) {
            var canFork = !!(msg.capabilities && msg.capabilities.threadFork);
            forkBtn.hidden = !canFork;
          }
          // Iter 6a: Codex skills picker capability flag. Hides the topbar
          // button, the inline picker, and the popover panel for Claude
          // projects (and codex projects on a CLI that doesn't advertise
          // skills/list).
          setCodexSkillsCapability(!!(msg.capabilities && msg.capabilities.codexSkills));
