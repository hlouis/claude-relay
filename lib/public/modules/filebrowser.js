...
var isRendered = false;      // rendered preview toggle state
...
export function initFileBrowser(_ctx) {
  ...
  document.getElementById("file-viewer-download").addEventListener("click", function () {
    ...
    var url = "api/file-download?path=" + encodeURIComponent(currentFilePath);
    var a = document.createElement("a");
    a.href = url;
    a.download = currentFilePath.replace(/^.*\//, "") || "download";
  ...
  document.getElementById("file-viewer-render").addEventListener("click", function () {
    if (currentContent == null || (!currentIsMarkdown && !currentIsHtml)) return;
...
function renderBody() {
  ...
  if (currentIsHtml) {
    if (isRendered) {
      ...
      iframe.className = "file-viewer-html-preview";
      // allow-scripts only; deliberately omit allow-same-origin so the iframe
      // stays in an opaque origin and cannot reach the parent app's cookies,
      // localStorage, or DOM. allow-top-navigation is also omitted so a
      // malicious page cannot redirect the parent window.
      iframe.setAttribute("sandbox", "allow-scripts");
      iframe.src = "api/file-preview/" + encodePreviewPath(currentFilePath);
      bodyEl.innerHTML = "";
      bodyEl.appendChild(iframe);
      renderBtn.classList.add("active");
      renderBtn.title = "Show source";
    } else {
      renderCodeWithLineNumbers(bodyEl, currentContent, "html");
      renderBtn.classList.remove("active");
      renderBtn.title = "Preview HTML";
    }
...
function showFileContent(msg) {
  ...
  if (msg.error) {
    ...
    currentIsHtml = (ext === "html" || ext === "htm");

    if (currentIsMarkdown || currentIsHtml) {
      renderBtn.classList.remove("hidden");
      renderBtn.title = currentIsHtml ? "Preview HTML" : "Render markdown";
    }

    // Show raw by default, use renderBody for preview toggles
    if (currentIsMarkdown || currentIsHtml) {
      renderBody();
    } else {
