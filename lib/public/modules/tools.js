...
function toolFullSummary(name, input) {
  if (!input || typeof input !== "object") return "";
  switch (name) {
...
export function renderAskUserQuestion(toolId, input) {
  ...
  questions.forEach(function (q, qIdx) {
    ...
    var otherComposing = false;
    otherInput.addEventListener("compositionstart", function () { otherComposing = true; });
    otherInput.addEventListener("compositionend", function () { otherComposing = false; });
    otherInput.addEventListener("keydown", function (e) {
      if (e.key === "Enter" && !e.shiftKey && !otherComposing) {
        e.preventDefault();
        submitAskUserAnswer(container, toolId, questions, answers, multiSelections);
      }
    });
...
export function renderPermissionRequest(requestId, toolName, toolInput, decisionReason, source) {
  ...
  if (source) container.dataset.source = source;

  // Header — surface the originating backend so users can tell at a glance
  // whether this prompt came from Claude or Codex (different sandbox model,
  // different "always allow" semantics).
  ...
  header.innerHTML =
    ...
    '<span class="permission-source-badge" data-source="' + (source === "codex" ? "codex" : "claude") + '">' + sourceLabel + '</span>';
  ...
  if (summaryText) {
    var permDescEl = summary.querySelector(".permission-tool-desc");
    permDescEl.textContent = summaryText;
    permDescEl.title = summaryText;
  }
...
export function createToolItem(id, name) {
  ...
  if (!currentToolGroup || currentToolGroup.closed) {
    ...
    groupEl.querySelector(".tool-group-header").addEventListener("click", function () {
      ...
      if (toolGroups[gid]) toolGroups[gid].userToggled = true;
    });
...
export function updateToolExecuting(id, name, input) {
  ...
  var fullDesc = toolFullSummary(name, input);
  if (fullDesc) descEl.title = fullDesc;
