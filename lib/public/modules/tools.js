...
function submitAskUserAnswer(container, toolId, questions, answers, multiSelections) {
  ...
  for (var i = 0; i < questions.length; i++) {
    ...
    if (q.multiSelect && multiSelections[i] && multiSelections[i].size > 0) {
      result[key] = Array.from(multiSelections[i]).join(", ");
    } else if (answers[i]) {
...
function showAnswerSummary(container, questions, answers) {
  ...
  for (var key in answers) {
    ...
    var isIndexKey = String(qi) === key;
    var label = (isIndexKey && questions && questions[qi] && questions[qi].question) ? questions[qi].question : key;
    if (!label || label === "undefined") label = "Answer";
  ...
  if (container && !container.classList.contains("answered")) {
    ...
    if (answers && Object.keys(answers).length > 0) {
      ...
      for (var key in answers) {
        ...
        if (String(idx) !== key) {
          idx = -1;
          for (var qki = 0; qki < questions.length; qki++) {
            if (questions[qki].question === key) {
              idx = qki;
              break;
            }
          }
