...
export function getWtCollapsedPref(slug) {
  // Returns raw preference (true/false) or undefined if never set
  return wtCollapsed[slug];
}

export function toggleWtCollapsed(slug, collapsed) {
  setWtCollapsed(slug, collapsed);
}
...
function showWorktreeModal(parentSlug, parentName) {
  ...
  branchInput.setAttribute("list", "wt-branch-options");
  modal.appendChild(branchInput);

  var branchList = document.createElement("datalist");
  branchList.id = "wt-branch-options";
  modal.appendChild(branchList);
  ...
  function normalizeBranchName(name) {
    if (!name) return "";
    var out = name.replace(/^remotes\//, "");
    if (out.indexOf("origin/") === 0) out = out.substring(7);
    return out;
  }

  function setExistingBranchMode() {
    var branch = branchInput.value.trim();
    var isExisting = !!existingBranches[branch];
    baseSelect.disabled = isExisting;
    baseLabel.textContent = isExisting ? "Base branch (not used for existing branch)" : "Base branch";
  }
  ...
  fetch("/p/" + parentSlug + "/api/branches")
    ...
    .then(function (data) {
      ...
      for (var i = 0; i < branches.length; i++) {
        var branchName = normalizeBranchName(branches[i]);
        if (!branchName || branchName === "HEAD" || branchName.indexOf("HEAD ->") !== -1) continue;
        if (!existingBranches[branchName]) {
          existingBranches[branchName] = true;
          normalized.push(branchName);
          var branchOpt = document.createElement("option");
          branchOpt.value = branchName;
          branchList.appendChild(branchOpt);
          var opt = document.createElement("option");
          opt.value = branchName;
          opt.textContent = branchName;
          if (branchName === defBranch) opt.selected = true;
      ...
      if (normalized.length === 0) {
        ...
        fallbackOpt.textContent = "main";
        baseSelect.appendChild(fallbackOpt);
      }
      setExistingBranchMode();
    })
    .catch(function () {});
  ...
  function doCreate() {
    ...
    var existingBranch = !!existingBranches[branch];
    var base = existingBranch ? null : (baseSelect.value.trim() || null);
  ...
  branchInput.addEventListener("input", setExistingBranchMode);
