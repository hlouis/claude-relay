...
var mentionFiltered = [];        // filtered candidate list for the active tab
var mentionActiveIdx = -1;       // highlighted item in dropdown
var mentionCurrentQuery = "";
var mentionActiveTab = "file";
var mentionTabTouched = false;
var mentionGroups = { file: [], mate: [], user: [] };
var fileCompleteEntries = [];
var fileCompleteDir = null;
var fileCompleteFilter = "";
var fileCompleteLoading = false;
...
export function showMentionMenu(query) {
  if (!mentionActive) {
    mentionActiveTab = "file";
    mentionTabTouched = false;
  }
  mentionCurrentQuery = query || "";
  requestFileCompletions(mentionCurrentQuery);
  renderMentionMenu();
}

function splitMentionCandidates(query) {
  ...
  var groups = { file: buildFileCandidates(), mate: [], user: [] };
  for (var i = 0; i < filtered.length; i++) {
    if (filtered[i].kind === "user") groups.user.push(filtered[i]);
    else groups.mate.push(filtered[i]);
  }
  return groups;
}

function requestFileCompletions(query) {
  var parsed = parseFileQuery(query || "");
  fileCompleteFilter = parsed.filter.toLowerCase();
  if (parsed.dir === fileCompleteDir) return;
  fileCompleteDir = parsed.dir;
  fileCompleteEntries = [];
  fileCompleteLoading = true;
  if (ctx && ctx.ws && ctx.connected) {
    ctx.ws.send(JSON.stringify({ type: "fs_list", path: parsed.dir, source: "at-complete" }));
  }
}

function parseFileQuery(query) {
  var dir = ".";
  var filter = query || "";
  var lastSlash = filter.lastIndexOf("/");
  if (lastSlash !== -1) {
    dir = filter.substring(0, lastSlash) || ".";
...
function buildFileCandidates() {
  if (fileCompleteLoading || fileCompleteEntries.length === 0) return [];
  var items = fileCompleteEntries.filter(function (entry) {
    if (!entry || !entry.name) return false;
    return !fileCompleteFilter || entry.name.toLowerCase().indexOf(fileCompleteFilter) !== -1;
  });
  ...
  return items.map(function (entry) {
    return {
      ...
      fileType: entry.type,
      id: entry.path,
      name: entry.name + (entry.type === "dir" ? "/" : ""),
      path: entry.path,
      color: entry.type === "dir" ? "#3b82f6" : "#64748b",
      bio: entry.path,
      raw: entry,
    };
  });
...
export function handleMentionFsListResult(msg) {
  if (!msg || msg.source !== "at-complete") return false;
  if ((msg.path || ".") !== fileCompleteDir) return true;
  fileCompleteLoading = false;
  fileCompleteEntries = msg.entries || [];
  if (mentionActive || mentionAtIdx >= 0) renderMentionMenu();
  return true;
}

function renderMentionMenu() {
  mentionGroups = splitMentionCandidates(mentionCurrentQuery);
  var tabs = [];
  if (mentionGroups.file.length > 0) tabs.push("file");
  if (mentionGroups.mate.length > 0) tabs.push("mate");
  if (mentionGroups.user.length > 0) tabs.push("user");
  ...
  if (!mentionTabTouched && tabs.indexOf("file") !== -1) {
    mentionActiveTab = "file";
  } else if (tabs.indexOf(mentionActiveTab) === -1) {
    mentionActiveTab = tabs.indexOf("file") !== -1 ? "file" : tabs[0];
  }

  mentionActive = true;
  mentionActiveIdx = 0;
  mentionFiltered = mentionGroups[mentionActiveTab] || [];
  ...
  menuEl.innerHTML =
    '<div class="mention-hint">' +
      '<span>' + hintTextForTab(mentionActiveTab) + '</span>' +
      '<button class="mention-close-btn" aria-label="Close">&times;</button>' +
    '</div>' +
    renderMentionTabs(tabs) +
    mentionFiltered.map(function (c, i) {
      if (c.kind === "file") return renderFileItem(c, i);
      return renderPersonItem(c, i);
    }).join("");
  ...
  menuEl.querySelectorAll(".mention-tab").forEach(function (el) {
    el.addEventListener("click", function (e) {
      e.preventDefault();
      e.stopPropagation();
      mentionTabTouched = true;
      mentionActiveTab = el.dataset.tab;
      renderMentionMenu();
    });
  });
...
function renderMentionTabs(tabs) {
  if (tabs.length <= 1) return "";
  return '<div class="mention-tabs">' + tabs.map(function (tab) {
    return '<button type="button" class="mention-tab' + (tab === mentionActiveTab ? ' active' : '') + '" data-tab="' + tab + '">' +
      tabLabel(tab) +
    '</button>';
  }).join("") + '</div>';
}
...
function tabLabel(tab) {
  ...
  if (tab === "mate") return "Mates";
  return "Users";
}

function hintTextForTab(tab) {
  if (tab === "file") return "Insert a project file path";
  if (tab === "mate") return "Mention a Mate";
  return "Mention a teammate";
}

function renderFileItem(c, i) {
  var icon = c.fileType === "dir" ? "folder" : "file";
  var typeBadge = c.fileType === "dir" ? "DIR" : "FILE";
  return '<div class="mention-item mention-file-item' + (i === 0 ? ' active' : '') + '" data-idx="' + i + '">' +
    '<div class="mention-item-avatar-wrap mention-file-icon">' + iconHtml(icon) + '</div>' +
    '<div class="mention-item-info">' +
      '<span class="mention-item-name">' + escapeHtml(c.name) + ' <span class="mention-item-badge">' + typeBadge + '</span></span>' +
      '<span class="mention-item-bio">' + escapeHtml(c.path) + '</span>' +
    '</div>' +
    '</div>';
}

function renderPersonItem(c, i) {
...
function selectFileItem(item) {
  if (!ctx.inputEl || mentionAtIdx < 0) return;
  var val = ctx.inputEl.value;
  var cursorPos = ctx.inputEl.selectionStart;
  var before = val.substring(0, mentionAtIdx + 1);
  var after = val.substring(cursorPos);
  var insertPath = item.path || "";
  if (item.fileType === "dir") {
    if (insertPath.charAt(insertPath.length - 1) !== "/") insertPath += "/";
    ctx.inputEl.value = before + insertPath + after;
    var dirCursor = mentionAtIdx + 1 + insertPath.length;
    ctx.inputEl.selectionStart = ctx.inputEl.selectionEnd = dirCursor;
    mentionCurrentQuery = insertPath;
    fileCompleteDir = null;
    requestFileCompletions(mentionCurrentQuery);
    renderMentionMenu();
  } else {
    ctx.inputEl.value = before + insertPath + " " + after;
    var fileCursor = mentionAtIdx + 1 + insertPath.length + 1;
    ctx.inputEl.selectionStart = ctx.inputEl.selectionEnd = fileCursor;
    hideMentionMenu();
  }
