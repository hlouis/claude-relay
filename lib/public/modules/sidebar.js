...
function renderSessionItem(s) {
  ...
  // Iter 4: badge sessions whose backend DIFFERS from the project default.
  // The signal is "this session is the odd one out" — in a single-backend
  // project (today's reality) this renders zero badges, so existing UIs
  // see no visual noise. After a user flips the project default, only the
  // sessions that pre-dated the flip light up. The badge text is the
  // backend's first two letters uppercased (CL / CO), mirroring the
  // existing project-name initials avatar — same visual vocabulary.
  var sessBackend = s.backend || null;
  var projDefault = ctx.currentDefaultBackend || null;
  if (sessBackend && projDefault && sessBackend !== projDefault) {
    var label = sessBackend.substring(0, 2).toUpperCase();
    var titleAttr = sessBackend === "codex" ? "Codex session" : "Claude session";
    textHtml += '<span class="session-backend-badge" data-backend="' + sessBackend + '" title="' + titleAttr + '">' + label + '</span>';
  }
...
function createSheetProjectItem(p, isWorktree) {
  var el = document.createElement("button");
  el.className = "mobile-project-item" + (p.slug === cachedCurrentSlug ? " active" : "") + (isWorktree ? " wt-item" : "");

  if (p.icon) {
    var emojiSpan = document.createElement("span");
    emojiSpan.className = "mobile-project-abbrev mobile-project-emoji";
  ...
  // Codex projects get an outline text badge so the user can tell which
  // backend a project belongs to at a glance (uses --accent2 per Clay's
  // design rules; --accent stays reserved for interactions).
  if (p.backend === "codex") {
    var bBadge = document.createElement("span");
    bBadge.className = "mobile-project-backend-badge";
    bBadge.textContent = "Codex";
    bBadge.title = "OpenAI Codex backend";
    el.appendChild(bBadge);
  }

  if (isWorktree && p.worktreeAccessible === false) {
    el.classList.add("wt-disabled");
...
function renderSheetProjects(listEl) {
  ...
  for (var i = 0; i < grouped.parents.length; i++) {
    ...
    if (worktrees.length === 0) {
      listEl.appendChild(createSheetProjectItem(p, false));
      continue;
    }

    // Folder: parent + worktrees with collapse/expand
    var folderDiv = document.createElement("div");
    folderDiv.className = "mobile-project-folder";
    if (wtCollapsed[p.slug]) folderDiv.classList.add("collapsed");

    var headerEl = createSheetProjectItem(p, false);
    ...
    for (var wi = 0; wi < worktrees.length; wi++) {
      wtList.appendChild(createSheetProjectItem(worktrees[wi], true));
    }
...
function createMobileProjectItem(p, currentSlug, isWorktree) {
  ...
  // Codex projects get an outline text badge so the user can tell which
  // backend a project belongs to at a glance. Uses --accent2 (info-ish)
  // rather than --accent (interactive) per Clay's design rules.
  if (p.backend === "codex") {
    var badge = document.createElement("span");
    badge.className = "mobile-project-backend-badge";
    badge.textContent = "Codex";
    badge.title = "OpenAI Codex backend";
