...
export function initProjectSettings(appCtx, emojiCategories) {
  ...
  // Iter 4: app.js's project_backend_changed handler calls this to
  // re-paint the backend toggle when another tab (or the local action)
  // flips the project default. Cheap to call — populateDefaults short-
  // circuits if the panel isn't open. Exposed via window for the same
  // reason ctx is exposed: project-settings is loaded as an ES module
  // and has no other communication channel back to app.js.
  window.__projectSettingsRefreshDefaults = function () {
...
// Iter 4: project default backend toggle. Always rendered regardless of
// which sub-block (claude / codex defaults) is showing — the toggle is
// what flips between them for FUTURE sessions, while the per-backend
// blocks below show the active session's view. Re-runs on every
// populateDefaults() call so a `project_backend_changed` broadcast
// re-paints the active button without manual refresh.
function populateBackendCard() {
  var bar = document.getElementById("ps-backend-bar");
  if (!bar) return;
  var current = (ctx.currentDefaultBackend || ctx.currentBackend || "claude");
  var btns = bar.querySelectorAll(".ps-backend-btn");
  for (var i = 0; i < btns.length; i++) {
    var b = btns[i];
    var isActive = b.dataset.value === current;
    b.classList.toggle("active", isActive);
    b.setAttribute("aria-checked", isActive ? "true" : "false");
    // Avoid stacking listeners across re-renders. Replace each button
    // with a fresh clone before binding so re-painting is idempotent.
    var clone = b.cloneNode(true);
    b.parentNode.replaceChild(clone, b);
    (function (btn) {
      btn.addEventListener("click", function () {
        var v = btn.dataset.value;
        if (!v || v === ctx.currentDefaultBackend) return;
        // Optimistic paint — server will confirm via project_backend_changed
        // and any roll-back becomes visible if the persistence step fails.
        var allBtns = bar.querySelectorAll(".ps-backend-btn");
        for (var k = 0; k < allBtns.length; k++) {
          var ab = allBtns[k];
          var on = ab.dataset.value === v;
          ab.classList.toggle("active", on);
          ab.setAttribute("aria-checked", on ? "true" : "false");
        }
        psSendMsg("set_project_backend", { backend: v });
      });
    })(clone);
  }
}
...
function populateDefaults() {
  // Iter 4: backend toggle is always shown. The per-backend defaults block
  // below it follows the ACTIVE session's backend (iter 3 invariant), so
  // a Claude session in a Codex-default project sees Claude controls — and
  // the user can still flip the project default from this same panel.
  populateBackendCard();

  // Iter 3 step 3: dispatch by backend. The Claude block is rendered exactly
  // as before; the Codex block renders the four controls Codex actually
  // honors (model, reasoning effort, sandbox, approval policy).
  if (ctx.currentBackend === "codex") {
    populateDefaultsCodex();
    return;
  }
...
// --- Codex defaults ---
//
// Iter 3 step 3 scope: declarative segmented controls + WS messages. Live
// model list comes in step 4 (model/list RPC); persistence to daemon.json
// is deferred — values stick on the codex backend instance and apply on the
// next thread/start, which is what users expect for sandbox/approval.

// Codex models we expose without contacting the API. Step 4 will swap this
// for a model/list RPC; until then we hard-code the common set so the
// dropdown is never empty. The first entry is the default Codex picks if
// no model param is supplied.
var CODEX_MODELS = [
  { value: "gpt-5-codex", desc: "Codex's flagship coding model." },
  { value: "gpt-5", desc: "General-purpose GPT-5." },
  { value: "gpt-4.1", desc: "Stable GPT-4.1." },
];

var CODEX_EFFORT = [
  { value: "low", desc: "Minimal reasoning. Fastest." },
  { value: "medium", desc: "Balanced reasoning." },
  { value: "high", desc: "Deep reasoning. Slowest." },
];

var CODEX_SANDBOX = [
  { value: "read-only", label: "Read-only", desc: "Block all writes and command execution outside read." },
  { value: "workspace-write", label: "Workspace write", desc: "Allow writes inside the project; ask before reaching outside." },
  { value: "danger-full-access", label: "Full access", desc: "No sandbox. Use with caution." },
];

var CODEX_APPROVAL = [
  { value: "untrusted", label: "Untrusted", desc: "Ask before any tool use." },
  { value: "on-failure", label: "On failure", desc: "Auto-approve until something fails, then ask." },
  { value: "on-request", label: "On request", desc: "Ask only when the agent escalates (matches Clay's permission flow)." },
  { value: "never", label: "Never", desc: "Auto-approve everything. Dangerous." },
];

function buildSegmentedBar(barId, options, currentValue, onPick) {
  ...
  for (var i = 0; i < options.length; i++) {
    (function (opt) {
      var btn = document.createElement("button");
      btn.className = "settings-btn-option" + (opt.value === currentValue ? " active" : "");
      btn.textContent = opt.label || (opt.value.charAt(0).toUpperCase() + opt.value.slice(1));
      btn.title = opt.desc || "";
...
function buildCodexModelList(currentModel) {
  var listEl = document.getElementById("ps-codex-model-list");
  if (!listEl) return;
  listEl.innerHTML = "";
  for (var i = 0; i < CODEX_MODELS.length; i++) {
    (function (m) {
      var item = document.createElement("div");
      item.className = "settings-model-item" + (m.value === currentModel ? " active" : "");
      item.dataset.model = m.value;

      var nameSpan = document.createElement("span");
      nameSpan.className = "settings-model-name";
      nameSpan.textContent = m.value;
      ...
      item.addEventListener("click", function () {
        psSendMsg("set_project_default_model", { model: m.value });
...
function populateDefaultsCodex() {
  buildCodexModelList(ctx.currentModel || "");
  buildSegmentedBar("ps-codex-effort-bar", CODEX_EFFORT, ctx.currentEffort || "medium", function (v) {
    psSendMsg("set_project_default_effort", { effort: v });
  });
  // Sandbox / approval defaults mirror codex-backend's hard-coded values
  // (workspace-write + on-request) until a server-side capability echo is
  // wired in step 4. The user pick is sent immediately and applied to the
  // next thread/start.
  var currentSandbox = (ctx.currentCodexConfig && ctx.currentCodexConfig.sandbox) || "workspace-write";
  var currentApproval = (ctx.currentCodexConfig && ctx.currentCodexConfig.approvalPolicy) || "on-request";
  buildSegmentedBar("ps-codex-sandbox-bar", CODEX_SANDBOX, currentSandbox, function (v) {
    psSendMsg("set_codex_sandbox", { sandbox: v });
  });
  buildSegmentedBar("ps-codex-approval-bar", CODEX_APPROVAL, currentApproval, function (v) {
    psSendMsg("set_codex_approval_policy", { approvalPolicy: v });
  });
}
