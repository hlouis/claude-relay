...
import { getRecord as getSessionMessageCache, beginHistoryCollect, collectHistoryMessage, finishHistoryCollect, cancelHistoryCollect, mergeAppend, appendLiveMessage, replaceWithHistory, messageFingerprint } from './session-message-cache.js';
...
import { handleMentionStart, handleMentionActivity, handleMentionStream, handleMentionDone, handleMentionError, handleMentionFsListResult, renderMentionUser, renderMentionResponse, renderUserMention } from './mention.js';
...
function sendHistoryRequest(msg) {
  var ws = getWs();
  if (!ws || ws.readyState !== 1) return;
  ws.send(JSON.stringify(msg));
}

function replayCachedHistory(rec) {
  if (!rec) return;
  store.set({ replayingCachedHistory: true });
  processMessage({ type: "history_meta", total: rec.total || rec.to || 0, from: rec.from || 0 });
  var items = rec.items || [];
  for (var i = 0; i < items.length; i++) {
    processMessage(items[i]);
  }
  var doneMsg = Object.assign({ type: "history_done" }, rec.doneMeta || {});
  processMessage(doneMsg);
  store.set({ replayingCachedHistory: false });
}

function requestHistoryForSession(sessionId, hasHistory) {
  var slug = store.get('currentSlug') || "default";
  var rec = getSessionMessageCache(slug, sessionId);
  if (rec && rec.items && rec.items.length > 0) {
    replayCachedHistory(rec);
    sendHistoryRequest({
      type: "load_more_history",
      after: rec.to,
      tailFingerprint: rec.tailFingerprint || messageFingerprint(rec.items[rec.items.length - 1]),
    });
    return;
  }
  if (hasHistory) {
    sendHistoryRequest({ type: "load_more_history", initial: true });
  } else {
    replaceWithHistory(slug, sessionId, { from: 0, to: 0, total: 0 }, [], {});
  }
}
...
export function processMessage(msg) {
    ...
    if (store.get('replayingHistory') && !store.get('replayingCachedHistory') && msg.type !== "history_done" && msg.type !== "history_meta" && msg.type !== "history_prepend" && msg.type !== "history_append" && msg.type !== "history_replace") {
      collectHistoryMessage(msg);
    } else if (msg._ts && !store.get('replayingHistory') && !store.get('replayingCachedHistory')) {
      appendLiveMessage(store.get('currentSlug') || "default", store.get('activeSessionId'), msg);
    }
    ...
    switch (msg.type) {
      case "history_meta":
        if (!store.get('replayingCachedHistory')) {
          beginHistoryCollect(store.get('currentSlug') || "default", store.get('activeSessionId'), msg);
        }
      ...
      case "history_append":
        ...
        for (var _hai = 0; _hai < (msg.items || []).length; _hai++) {
          processMessage(msg.items[_hai]);
        }
        ...
        mergeAppend(store.get('currentSlug') || "default", store.get('activeSessionId'), msg.items || [], msg.meta || {}, msg.doneMeta || null);
        if (msg.meta && typeof msg.meta.total === "number") {
          store.set({ historyTotal: msg.meta.total });
        }
        break;
      ...
      case "history_replace":
        ...
        resetClientState();
        replaceWithHistory(store.get('currentSlug') || "default", store.get('activeSessionId'), msg.meta || {}, msg.items || [], msg.doneMeta || {});
        store.set({ replayingCachedHistory: true });
        processMessage({ type: "history_meta", total: (msg.meta && msg.meta.total) || 0, from: (msg.meta && msg.meta.from) || 0 });
        for (var _hri = 0; _hri < (msg.items || []).length; _hri++) {
          processMessage(msg.items[_hri]);
        }
        processMessage(Object.assign({ type: "history_done" }, msg.doneMeta || {}));
        store.set({ replayingCachedHistory: false });
        break;

      case "history_done":
        if (!store.get('replayingCachedHistory')) {
          finishHistoryCollect(msg);
        }
        store.set({ replayingHistory: false });
      ...
      case "session_switched":
        ...
        clearRemoteCursors();
        resetClientState();
        requestHistoryForSession(msg.id, !!msg.hasHistory);
      ...
      case "fs_list_result":
        if (handleMentionFsListResult(msg)) break;
        handleFsList(msg);
        break;
