...
import { maybeShowInlineMenu as maybeShowCodexSkillMenu, hideInlineMenu as hideCodexSkillMenu, isInlineMenuVisible as isCodexSkillMenuVisible, moveInlineSelection as moveCodexSkillSelection, commitInlineSelection as commitCodexSkillSelection } from './codex-skills.js';
...
// --- Slash menu ---
//
// Iter 4 follow-up: filter Claude-only commands when the active session is
// Codex. Today only `/rewind` is Claude-specific (codex has its own
// thread-fork affordance via the topbar Fork button, not a slash command).
// The dynamic `slashCommands` come from the Claude SDK only — codex doesn't
// emit a `slash_commands` frame — so they're naturally absent on Codex.
// Universal commands (/clear, /context, /usage, /status) are kept.
var CLAUDE_ONLY_COMMANDS = { rewind: true };

function getAllCommands() {
  var backend = (typeof ctx.currentBackend === "function") ? ctx.currentBackend() : null;
  var builtins = builtinCommands;
  if (backend === "codex") {
    builtins = builtins.filter(function (c) { return !CLAUDE_ONLY_COMMANDS[c.name]; });
  }
  return builtins.concat(ctx.slashCommands());
}
...
export function initInput(_ctx) {
  ...
  ctx.inputEl.addEventListener("input", function () {
    ...
    if (ctx.hideSuggestionChips) ctx.hideSuggestionChips();
    var val = ctx.inputEl.value;
    // Iter 6a: Codex skill autocomplete. Tries first; if it shows (i.e.
    // we're in a Codex project AND cursor is on a `$<...>` or `/<...>`
    // token), suppress the legacy Claude slash-menu so the two pickers
    // never collide. For Claude projects maybeShowCodexSkillMenu returns
    // false (capability gate inside the module) and the slash-menu logic
    // runs unchanged.
    var codexInline = maybeShowCodexSkillMenu();
    if (codexInline) {
      hideSlashMenu();
    } else if (val.startsWith("/") && !val.includes(" ") && val.length > 1) {
  ...
  ctx.inputEl.addEventListener("keydown", function (e) {
    // Iter 6a: codex skill inline picker — shares the slash-menu key
    // contract (Up/Down/Tab/Enter/Esc) but operates on its own DOM and
    // selects $<name> instead of /<name>. Only active for Codex projects
    // (the module gates itself).
    if (isCodexSkillMenuVisible()) {
      if (e.key === "ArrowDown") { e.preventDefault(); moveCodexSkillSelection(1); return; }
      if (e.key === "ArrowUp")   { e.preventDefault(); moveCodexSkillSelection(-1); return; }
      if (e.key === "Tab" || (e.key === "Enter" && !e.shiftKey)) {
        e.preventDefault();
        commitCodexSkillSelection();
        return;
      }
      if (e.key === "Escape") { e.preventDefault(); hideCodexSkillMenu(); return; }
    }
