...
import { appendOptimisticUserMessage } from './session-message-cache.js';
...
function isSoftKeyboardLikelyOpen() {
  ...
  var viewportHeight = window.visualViewport.height || 0;
  var layoutHeight = window.innerHeight || document.documentElement.clientHeight || 0;
  if (!viewportHeight || !layoutHeight) return true;
...
export function sendMessage() {
  ...
  var clientMessageId = "cm_" + optimisticTs + "_" + Math.random().toString(36).slice(2);
  ctx.currentMsgTs = optimisticTs;
  ...
  appendOptimisticUserMessage(store.get('currentSlug') || "default", store.get('activeSessionId'), {
    type: "user_message",
    text: text || "",
    images: images.length > 0 ? images : null,
    pastes: pastes.length > 0 ? pastes : null,
...
export function initInput(_ctx) {
  ...
  ctx.inputEl.addEventListener("keydown", function (e) {
    ...
    if (e.key === "Enter" && !e.shiftKey && !isComposing) {
      // Mobile soft keyboard: Enter inserts newline, send via button only.
      if ("ontouchstart" in window && isSoftKeyboardLikelyOpen()) {
        return;
      }
      e.preventDefault();
