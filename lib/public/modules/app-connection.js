...
var reconnectOverlayDelayId = null;
var hasConnectedOnce = false;
var suppressNextReconnectOverlay = false;

function clearReconnectOverlayDelay() {
  if (reconnectOverlayDelayId) {
    clearTimeout(reconnectOverlayDelayId);
    reconnectOverlayDelayId = null;
  }
}
...
function scheduleReconnectOverlay() {
  clearReconnectOverlayDelay();
  reconnectOverlayDelayId = setTimeout(function () {
    reconnectOverlayDelayId = null;
...
export function suppressReconnectOverlayOnce() {
  suppressNextReconnectOverlay = true;
  hideConnectOverlay();
}
  ...
  store.subscribe(function (state, prev) {
    ...
    if (state.connected !== prev.connected) {
      ...
      if (state.connected) {
        ...
        suppressNextReconnectOverlay = false;
        ...
        if (!hasConnectedOnce) {
          if (connectOverlay) connectOverlay.classList.remove("hidden");
          startLogoAnimation();
        } else if (suppressNextReconnectOverlay) {
          suppressNextReconnectOverlay = false;
          hideConnectOverlay();
        } else {
