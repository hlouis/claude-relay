...
function renderSheetSessions(listEl) {
  ...
  // Build chips: parents (each followed by its worktrees), then mates
  var chips = [];
  var WT_AUTO_COLLAPSE_THRESHOLD = 3;
  var grouped = groupProjectsByParent(getCachedProjectList());
  var isDmActive = document.body.classList.contains("mate-dm-active");

  function buildParentChip(p, worktrees) {
    ...
    if (worktrees.length > 0) chip.dataset.hasWt = String(worktrees.length);
  ...
  function buildWorktreeChip(wt, parentSlug) {
    var chip = document.createElement("button");
    chip.className = "mobile-chat-chip mobile-chat-chip-wt";
    if (wt.slug === getCachedCurrentSlug() && !isDmActive) chip.classList.add("active");
    chip.dataset.type = "project";
    chip.dataset.slug = wt.slug;
    chip.dataset.wtParent = parentSlug;

    var branchIcon = document.createElement("span");
    branchIcon.className = "mobile-chat-chip-wt-icon";
    branchIcon.innerHTML = iconHtml("git-branch");
    chip.appendChild(branchIcon);

    var label = document.createElement("span");
    var branchName = wt.slug.indexOf("--") !== -1
      ? wt.slug.substring(wt.slug.indexOf("--") + 2)
      : (wt.name || wt.slug);
    ...
    var statusDot = document.createElement("span");
    statusDot.className = "icon-strip-status";
    if (wt.isProcessing) statusDot.classList.add("processing");
    chip.appendChild(statusDot);

    if (wt.unread > 0 && wt.slug !== getCachedCurrentSlug()) {
      var badge = document.createElement("span");
      badge.className = "mobile-chat-chip-badge";
      badge.textContent = wt.unread > 99 ? "99+" : String(wt.unread);
      chip.appendChild(badge);
    }
  ...
  function buildWtToggle(parentSlug, count, isCollapsed) {
    var toggle = document.createElement("span");
    toggle.className = "mobile-chat-chip-wt-toggle";
    if (isCollapsed) toggle.classList.add("collapsed");
    toggle.dataset.wtToggle = parentSlug;
    toggle.innerHTML = iconHtml("git-branch")
      + '<span class="mobile-chat-chip-wt-count">' + count + '</span>'
  ...
  for (var ci = 0; ci < grouped.parents.length; ci++) {
    (function (p) {
      ...
      var pref = getWtCollapsedPref(p.slug);
      var isCollapsed = pref === undefined
        ? worktrees.length >= WT_AUTO_COLLAPSE_THRESHOLD
        : !!pref;
      // Never collapse the group containing the currently-active worktree
      if (isCollapsed && !isDmActive) {
        for (var ai = 0; ai < worktrees.length; ai++) {
          if (worktrees[ai].slug === getCachedCurrentSlug()) { isCollapsed = false; break; }
        }
      }

      var parentChip = buildParentChip(p, worktrees);
      if (worktrees.length > 0) {
        parentChip.appendChild(buildWtToggle(p.slug, worktrees.length, isCollapsed));
      }
      chips.push(parentChip);

      for (var wi = 0; wi < worktrees.length; wi++) {
        var wtChip = buildWorktreeChip(worktrees[wi], p.slug);
        if (isCollapsed) wtChip.classList.add("wt-hidden");
        // Mark last worktree in group so CSS can restore gap after it
        if (wi === worktrees.length - 1) wtChip.classList.add("wt-tail");
        chips.push(wtChip);
      }
    })(grouped.parents[ci]);
  }
  ...
  // Ensure the currently active chip is visible in the horizontal scroll viewport.
  // Defer to next frame so the sheet's open transition does not interfere with layout.
  requestAnimationFrame(function () {
    var activeChip = filterBar.querySelector(".mobile-chat-chip.active");
    if (!activeChip) return;
    var barRect = filterBar.getBoundingClientRect();
    var chipRect = activeChip.getBoundingClientRect();
    var chipCenter = (chipRect.left + chipRect.right) / 2;
    var barCenter = (barRect.left + barRect.right) / 2;
    var delta = chipCenter - barCenter;
    var target = filterBar.scrollLeft + delta;
    var maxScroll = filterBar.scrollWidth - filterBar.clientWidth;
    if (target < 0) target = 0;
    if (target > maxScroll) target = maxScroll;
    filterBar.scrollTo({ left: target, behavior: "auto" });
  });
  ...
  for (var j = 0; j < chips.length; j++) {
    (function (chip) {
      chip.addEventListener("click", function (ev) {
        // If clicked the worktree toggle on a parent chip, just toggle collapse
        var toggleEl = ev.target && ev.target.closest && ev.target.closest(".mobile-chat-chip-wt-toggle");
        if (toggleEl && chip.contains(toggleEl)) {
          ev.stopPropagation();
          var parentSlug = toggleEl.dataset.wtToggle;
          var willCollapse = !toggleEl.classList.contains("collapsed");
          toggleEl.classList.toggle("collapsed", willCollapse);
          toggleWtCollapsed(parentSlug, willCollapse);
          var wtChips = filterBar.querySelectorAll('.mobile-chat-chip-wt[data-wt-parent="' + parentSlug + '"]');
          for (var w = 0; w < wtChips.length; w++) {
            wtChips[w].classList.toggle("wt-hidden", willCollapse);
          }
          return;
        }
