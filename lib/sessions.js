...
function createSessionManager(opts) {
  ...
  // session is born. The returned value is stamped into the session and
  // becomes immutable for that session's lifetime. project.backend may
  // change later (iter 4); the getter shape lets that change land without
  // re-threading APIs. Default "claude" preserves legacy single-backend
  // behavior for any caller that hasn't been updated.
  var getProjectBackend = (typeof opts.getProjectBackend === "function")
    ? opts.getProjectBackend
    : function () { return "claude"; };

  // Iter 3: looks up a backend's capability declaration so session_switched
  // can carry the right caps for re-gating UI when sessions of different
  // backends coexist in one project. Defaulting to null is intentional —
  // the frontend's pre-iter-3 fallback path (last-info caps) still works
  // when this resolver isn't wired in.
  var getCapabilitiesFor = (typeof opts.getCapabilitiesFor === "function")
    ? opts.getCapabilitiesFor
  ...
  function saveSessionFile(session) {
    ...
    try {
      ...
      // session.backend is stamped on creation and never mutated. Persisting
      // it freezes the session's backend identity to disk so a later flip of
      // project.backend cannot misclassify historical sessions.
      if (session.backend) metaObj.backend = session.backend;
  ...
  // One-shot migration helper: rewrite ONLY the meta line of an existing
  // JSONL, preserving the rest of the file and the original mtime so the
  // sidebar's "lastActivity" ordering isn't disturbed. Used to backfill the
  // backend field on legacy sessions that predate session-level backend.
  function rewriteMetaPreservingMtime(cliSessionId, newMeta, originalMtime) {
    var sfPath = sessionFilePath(cliSessionId);
    try {
      var existing = fs.readFileSync(sfPath, "utf8");
      var nlIdx = existing.indexOf("\n");
      // No newline → file has only the meta line (no history yet). Replace whole.
      var rest = nlIdx >= 0 ? existing.substring(nlIdx) : "\n";
      fs.writeFileSync(sfPath, JSON.stringify(newMeta) + rest);
      if (process.platform !== "win32") {
        try { fs.chmodSync(sfPath, 0o600); } catch (e) {}
      }
      if (originalMtime) {
        try {
          var t = new Date(originalMtime);
          fs.utimesSync(sfPath, t, t);
        } catch (e) {}
      }
    } catch (e) {
      console.error("[sessions] Meta rewrite failed for " + cliSessionId + ":", e.message);
    }
  }
  ...
  function loadSessions() {
    ...
    // Lazy migration: any session whose meta lacks `backend` gets stamped
    // with the project's CURRENT default at load time. We resolve once per
    // load, not per session — within one load all migrated sessions share
    // the same fallback backend. This is correct because legacy sessions all
    // belong to the same project and that project has a single current
    // default. Idempotent: subsequent loads see meta.backend and skip.
    var migrationFallback = null;
    var migratedIds = [];
    ...
    for (var i = 0; i < loaded.length; i++) {
      ...
      var sessionBackend;
      if (m.backend) {
        sessionBackend = m.backend;
      } else {
        if (migrationFallback === null) {
          try { migrationFallback = getProjectBackend() || "claude"; }
          catch (e) { migrationFallback = "claude"; }
        }
        sessionBackend = migrationFallback;
        migratedIds.push({ cliSessionId: m.cliSessionId, mtime: loaded[i].mtime });
      }
    ...
    // Persist migrations to disk after the in-memory load so a second
    // restart doesn't re-trigger migration. We rewrite ONLY the meta line
    // (history may be huge) and preserve mtime so sidebar order is stable.
    if (migratedIds.length > 0) {
      for (var mi = 0; mi < migratedIds.length; mi++) {
        var entry = migratedIds[mi];
        // Find the matching loaded record so we can build a precise meta.
        for (var li = 0; li < loaded.length; li++) {
          if (loaded[li].meta.cliSessionId !== entry.cliSessionId) continue;
          var newMeta = Object.assign({}, loaded[li].meta, { backend: migrationFallback });
          rewriteMetaPreservingMtime(entry.cliSessionId, newMeta, entry.mtime);
          break;
        }
      }
      console.log("[sessions] Backfilled backend=" + migrationFallback + " on " + migratedIds.length + " legacy session(s)");
    }
  }
  ...
  function mapSessionForClient(s, clientActiveId, wsUnread) {
    ...
    return {
      ...
      // Iter 3: surface the session's frozen backend identity so the
      // sidebar can badge sessions and the frontend can render the right
      // UI affordances when the user picks a session whose backend differs
      // from the project default. May be null only for the brief window
      // before a brand-new session has been stamped (defensive — iter 1's
      // createSession always stamps before this map runs).
  ...
  function createSession(sessionOpts, targetWs) {
    var localId = nextLocalId++;
    // Backend resolution rules (immutable for the session's lifetime):
    //   1. Explicit sessionOpts.backend wins — used by fork to inherit the
    //      source session's backend regardless of project's current default.
    //   2. Otherwise stamp the project's CURRENT default at this moment.
    // Once stamped, session.backend is treated as read-only by all callers.
    var explicitBackend = sessionOpts && sessionOpts.backend;
    var sessionBackend = explicitBackend || getProjectBackend() || "claude";
    var session = {
      localId: localId,
      queryInstance: null,
      messageQueue: null,
  ...
  function switchSession(localId, targetWs, resumeLastSeq) {
    ...
    // Iter 3: include the session's frozen backend AND its capability
    // declaration so the frontend can re-target body.backend-codex,
    // hide/show fork & skills affordances, and re-render the settings
    // drawer to match this session's backend — without a follow-up
    // round-trip. Capabilities may be null when the resolver isn't
    // wired in (older callers / tests); the frontend falls back to
    // the last-known info caps in that case.
    var _switchCaps = null;
    if (session.backend) { try { _switchCaps = getCapabilitiesFor(session.backend); } catch (e) {} }
    _send({ type: "session_switched", id: localId, cliSessionId: session.cliSessionId || null, loop: session.loop || null, backend: session.backend || null, capabilities: _switchCaps });
    broadcastSessionList();

    // Incremental replay if client has cached messages up to resumeLastSeq
    var total = session.history.length;
    var canResume = typeof resumeLastSeq === "number" && resumeLastSeq >= 0 && resumeLastSeq <= total;
    if (canResume) {
      _send({ type: "history_meta", total: total, from: resumeLastSeq, resumed: true });
      for (var ri = resumeLastSeq; ri < total; ri++) {
        session.history[ri].seq = ri;
        _send(session.history[ri]);
      }
      // Include last usage data for context panel
      var doneMsg = { type: "history_done" };
      for (var dj = total - 1; dj >= 0; dj--) {
        if (session.history[dj].type === "result") {
          var dr = session.history[dj];
          doneMsg.lastUsage = dr.usage || null;
          doneMsg.lastModelUsage = dr.modelUsage || null;
          doneMsg.lastCost = dr.cost != null ? dr.cost : null;
          doneMsg.lastStreamInputTokens = dr.lastStreamInputTokens || null;
          break;
        }
      }
      _send(doneMsg);
  ...
  function doSendAndRecord(session, obj) {
    obj.seq = session.history.length;
  ...
  function resumeSession(cliSessionId, opts, targetWs) {
    ...
    var localId = nextLocalId++;
    // resumeSession is the SDK-driven reattach path (Claude side). The
    // resumed session's backend is whatever the caller declares, falling
    // back to the project's current default. Same immutability rules apply.
    var explicitBackend = opts && opts.backend;
    var sessionBackend = explicitBackend || getProjectBackend() || "claude";
    var session = {
      localId: localId,
      queryInstance: null,
