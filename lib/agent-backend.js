...
function getBackendCapabilities(backend) {
  ...
  if (name === "claude") {
    return {
      settings: (sdkBridge.SUPPORTED_SETTINGS || []).slice(),
      // Iter 5b: Claude has rewind (in-place truncation), not fork.
      threadFork: false,
      // Iter 6a: Claude's skills system is wired into a different UI
      // (#skills-btn / lib/public/modules/skills.js) and uses Claude SDK
      // wire calls, not codex's `skills/list`. False here means "don't
      // show the codex skill picker for this project".
      codexSkills: false,
    };
  }
  if (name === "codex") {
    return {
      settings: (codexBackend.SUPPORTED_SETTINGS || []).slice(),
      // Iter 5b: HEAD-only thread fork available when this is true.
      // Frontend uses it to gate the topbar Fork button without having
      // to test for `body.backend-codex` directly.
      threadFork: true,
      // Iter 6a: gates the Codex-side skill picker button + popover.
      // The actual list comes from `codex_skills` WS frames; this flag
      // only governs UI visibility.
      codexSkills: true,
    };
  }
  return { settings: [] };
