...
// Iter 4 follow-up: per-project Codex config persistence factory.
// Returns a callback the project context invokes whenever the user changes
// sandbox / approvalPolicy / model / effort. We mutate the in-memory
// `config` and write daemon.json. Filtering on slug guards against stale
// callbacks from a removed project (a removed slug is a no-op).
function makePersistCodexConfig(slug) {
  return function (snapshot) {
    if (!snapshot || typeof snapshot !== "object") return;
    var dirty = false;
    for (var i = 0; i < config.projects.length; i++) {
      if (config.projects[i].slug !== slug) continue;
      var existing = config.projects[i].codexConfig || {};
      var next = {
        sandbox: snapshot.sandbox || existing.sandbox || null,
        approvalPolicy: snapshot.approvalPolicy || existing.approvalPolicy || null,
        model: snapshot.model || existing.model || null,
        effort: snapshot.effort || existing.effort || null,
      };
      // Strip nulls so daemon.json stays minimal for projects that have
      // never customized a key.
      var clean = {};
      var keys = Object.keys(next);
      for (var k = 0; k < keys.length; k++) {
        if (next[keys[k]]) clean[keys[k]] = next[keys[k]];
      }
      var prev = JSON.stringify(existing);
      var nxt = JSON.stringify(clean);
      if (prev !== nxt) {
        if (Object.keys(clean).length === 0) delete config.projects[i].codexConfig;
        else config.projects[i].codexConfig = clean;
        dirty = true;
      }
      break;
    }
    if (dirty) {
      ...
      catch (e) { console.error("[daemon] saveConfig failed for codexConfig update:", e.message || e); }
    }
  };
}
...
var relay = createServer({
  ...
  onAddProject: function (absPath, wsUser, options) {
    ...
    var backendChoice = (options && options.backend) || "claude";
    relay.addProject(absPath, slug, null, null, null, null, backendChoice, null, makePersistCodexConfig(slug));
    var projectEntry = { path: absPath, slug: slug, addedAt: Date.now(), backend: backendChoice };
    ...
    scanAndRegisterWorktrees(absPath, slug, null, wsUser && wsUser.id && wsUser.role !== "admin" ? wsUser.id : null, backendChoice);
  ...
  onCreateProject: function (projectName, wsUser, options) {
    ...
    // Register project
    var createBackend = (options && options.backend) || "claude";
    var projectEntry = { path: targetDir, slug: slug, addedAt: Date.now(), backend: createBackend };
    if (wsUser && wsUser.id) {
    ...
    relay.addProject(targetDir, slug, null, null, projectEntry.ownerId || null, null, createBackend, null, makePersistCodexConfig(slug));
    config.projects.push(projectEntry);
    saveConfig(config);
    try { syncClayrc(config.projects); } catch (e) {}
    console.log("[daemon] Created project:", slug, "→", targetDir, "entry:", JSON.stringify({ ownerId: projectEntry.ownerId, visibility: projectEntry.visibility, backend: createBackend }));
  ...
  onCloneProject: function (cloneUrl, wsUser, callback, options) {
    var os = require("os");
    ...
    proc.on("close", function (code) {
      ...
      // Register project
      var cloneBackend = (options && options.backend) || "claude";
      var projectEntry = { path: targetDir, slug: slug, addedAt: Date.now(), backend: cloneBackend };
      if (wsUser && wsUser.id) {
        if (config.osUsers || wsUser.role !== "admin") {
          projectEntry.ownerId = wsUser.id;
        }
      ...
      relay.addProject(targetDir, slug, null, null, projectEntry.ownerId || null, null, cloneBackend, null, makePersistCodexConfig(slug));
      config.projects.push(projectEntry);
      saveConfig(config);
      try { syncClayrc(config.projects); } catch (e) {}
  ...
  onRemoveProject: function (slug, userId) {
    ...
    if (isWorktreeSlug(slug)) {
      ...
      // Find worktree absolute path from registry
      var wtAbsPath = null;
      var entries = worktreeRegistry[wtParent] || [];
      for (var ei = 0; ei < entries.length; ei++) {
        if (entries[ei].slug === slug) { wtAbsPath = entries[ei].path; break; }
      }
      ...
      if (parentProject && wtAbsPath) {
        var rmResult = removeWorktree(parentProject.path, wtAbsPath);
        if (!rmResult.ok) {
      ...
      if (worktreeRegistry[wtParent]) {
        worktreeRegistry[wtParent] = worktreeRegistry[wtParent].filter(function (e) { return e.slug !== slug; });
      }
  ...
  // Iter 4: persist a project's default backend. The closure-side mutation
  // already happened in project.js (so getProjectBackend returns the new
  // value for any subsequent createSession); this just freezes it to disk
  // so a daemon restart picks up the user's choice. Validation is done at
  // the project handler — we trust the slug/value here.
  onSetProjectBackend: function (slug, backend) {
    if (backend !== "claude" && backend !== "codex") {
      return { ok: false, error: "Invalid backend: " + backend };
    }
    for (var i = 0; i < config.projects.length; i++) {
      if (config.projects[i].slug === slug) {
        config.projects[i].backend = backend;
        saveConfig(config);
        return { ok: true };
      }
    }
  ...
  onCreateWorktree: function (parentSlug, branchName, baseBranch) {
    ...
    var wtSlug = parentSlug + "--" + branchName.replace(/\//g, "-");
    var wtMeta = { parentSlug: parentSlug, branch: branchName, accessible: true };
    relay.addProject(result.path, wtSlug, branchName, parent.icon, parent.ownerId, wtMeta, parent.backend, null, makePersistCodexConfig(wtSlug));
    if (!worktreeRegistry[parentSlug]) worktreeRegistry[parentSlug] = [];
    worktreeRegistry[parentSlug].push({ slug: wtSlug, path: result.path });
    // Ensure rescan timer is running so stale worktrees get cleaned up
    if (!worktreeTimers[parentSlug]) {
      worktreeTimers[parentSlug] = setInterval(function () {
        rescanWorktrees(parent.path, parentSlug, parent.icon, parent.ownerId, parent.backend);
      }, 10000);
    }
...
// --- Worktree tracking ---
var worktreeRegistry = {}; // parentSlug -> [{ slug, path }, ...]
...
function scanAndRegisterWorktrees(parentPath, parentSlug, parentIcon, parentOwnerId, parentBackend) {
  ...
  for (var i = 0; i < worktrees.length; i++) {
    var wt = worktrees[i];
    var wtSlug = parentSlug + "--" + wt.slugSuffix;
    // Skip if already registered
    var alreadyRegistered = false;
    for (var j = 0; j < worktreeRegistry[parentSlug].length; j++) {
      if (worktreeRegistry[parentSlug][j].slug === wtSlug) { alreadyRegistered = true; break; }
    }
    if (alreadyRegistered) continue;
    var wtMeta = { parentSlug: parentSlug, branch: wt.branch || wt.dirName, accessible: wt.accessible };
    relay.addProject(wt.path, wtSlug, wt.branch || wt.dirName, parentIcon, parentOwnerId, wtMeta, parentBackend, null, makePersistCodexConfig(wtSlug));
    worktreeRegistry[parentSlug].push({ slug: wtSlug, path: wt.absPath });
    console.log("[daemon] Registered worktree:", wtSlug, "->", wt.path);
  }
  // Always start periodic rescan so CLI-created worktrees are auto-discovered
  if (!worktreeTimers[parentSlug]) {
    worktreeTimers[parentSlug] = setInterval(function () {
      rescanWorktrees(parentPath, parentSlug, parentIcon, parentOwnerId, parentBackend);
    }, 10000);
  }
}

function rescanWorktrees(parentPath, parentSlug, parentIcon, parentOwnerId, parentBackend) {
  if (worktreeScanning[parentSlug]) return;
  worktreeScanning[parentSlug] = true;
  try {
    var discovered = scanWorktrees(parentPath);
    var changed = false;
    var existingEntries = worktreeRegistry[parentSlug] || [];
    // Build set of discovered absolute paths
    var discoveredPaths = {};
    for (var i = 0; i < discovered.length; i++) {
      discoveredPaths[discovered[i].absPath] = discovered[i];
    }
    // Add new worktrees
    for (var di = 0; di < discovered.length; di++) {
      ...
      for (var ei = 0; ei < existingEntries.length; ei++) {
        if (existingEntries[ei].path === wt.absPath) { found = true; break; }
      }
      if (!found) {
        var wtMeta = { parentSlug: parentSlug, branch: wt.branch || wt.dirName, accessible: wt.accessible };
        relay.addProject(wt.path, wtSlug, wt.branch || wt.dirName, parentIcon, parentOwnerId, wtMeta, parentBackend, null, makePersistCodexConfig(wtSlug));
    ...
    // Remove stale worktrees (no longer reported by git)
    for (var si = existingEntries.length - 1; si >= 0; si--) {
      var entry = existingEntries[si];
      if (!discoveredPaths[entry.path]) {
        relay.removeProject(entry.slug);
        existingEntries.splice(si, 1);
        console.log("[daemon] Rescan: removed stale worktree:", entry.slug);
        changed = true;
      }
    }
    if (changed) {
...
function cleanupWorktreesForParent(parentSlug) {
  var wtEntries = worktreeRegistry[parentSlug] || [];
  for (var i = 0; i < wtEntries.length; i++) {
    relay.removeProject(wtEntries[i].slug);
    console.log("[daemon] Cascade removed worktree:", wtEntries[i].slug);
  }
...
for (var i = 0; i < projects.length; i++) {
  ...
  if (fs.existsSync(p.path)) {
    ...
    relay.addProject(p.path, p.slug, p.title, p.icon, p.ownerId, null, p.backend, p.codexConfig || null, makePersistCodexConfig(p.slug));
    // Discover and register worktrees for this project
    scanAndRegisterWorktrees(p.path, p.slug, p.icon, p.ownerId, p.backend);
  } else {
...
var ipc = createIPCServer(socketPath(), function (msg) {
  switch (msg.cmd) {
    case "add_project": {
      ...
      var slugs = config.projects.map(function (p) { return p.slug; });
      var slug = generateSlug(absPath, slugs);
      var ipcBackend = (msg.backend === "codex") ? "codex" : "claude";
      relay.addProject(absPath, slug, null, null, null, null, ipcBackend, null, makePersistCodexConfig(slug));
      config.projects.push({ path: absPath, slug: slug, addedAt: Date.now(), backend: ipcBackend });
      saveConfig(config);
      try { syncClayrc(config.projects); } catch (e) {}
      console.log("[daemon] Added project:", slug, "→", absPath, "backend=" + ipcBackend);
      // Discover and register worktrees for the new project
      scanAndRegisterWorktrees(absPath, slug, null, null, ipcBackend);
      relay.broadcastAll({
        type: "projects_updated",
        projects: relay.getProjects(),
        projectCount: config.projects.length,
    ...
    case "update": {
      ...
      try {
        ...
        var binPath = execSyncUpd(
          "npx --yes --package=@hlouis/clay@" + updTag + " -- which clay",
