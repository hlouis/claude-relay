...
var relay = createServer({
  ...
  onAddProject: function (absPath, wsUser, options) {
    ...
    relay.addProject(absPath, slug, null, null, null, null, backendChoice);
  ...
  onCreateProject: function (projectName, wsUser, options) {
    ...
    relay.addProject(targetDir, slug, null, null, projectEntry.ownerId || null, null, createBackend);
    config.projects.push(projectEntry);
    saveConfig(config);
    try { syncClayrc(config.projects); } catch (e) {}
  ...
  onCloneProject: function (cloneUrl, wsUser, callback, options) {
    ...
    proc.on("close", function (code) {
      ...
      relay.addProject(targetDir, slug, null, null, projectEntry.ownerId || null, null, cloneBackend);
      config.projects.push(projectEntry);
      saveConfig(config);
      try { syncClayrc(config.projects); } catch (e) {}
  ...
  onCreateWorktree: function (parentSlug, branchName, baseBranch) {
    ...
    relay.addProject(result.path, wtSlug, branchName, parent.icon, parent.ownerId, wtMeta, parent.backend);
...
function scanAndRegisterWorktrees(parentPath, parentSlug, parentIcon, parentOwnerId, parentBackend) {
  ...
  for (var i = 0; i < worktrees.length; i++) {
    ...
    relay.addProject(wt.path, wtSlug, wt.branch || wt.dirName, parentIcon, parentOwnerId, wtMeta, parentBackend);
...
for (var i = 0; i < projects.length; i++) {
  ...
  if (fs.existsSync(p.path)) {
    ...
    relay.addProject(p.path, p.slug, p.title, p.icon, p.ownerId, null, p.backend);
...
var ipc = createIPCServer(socketPath(), function (msg) {
  switch (msg.cmd) {
    case "add_project": {
      ...
      relay.addProject(absPath, slug, null, null, null, null, ipcBackend);
