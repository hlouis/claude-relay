...
function fetchVersion(channel) {
  ...
  return new Promise(function (resolve) {
    var req = https.get("https://registry.npmjs.org/@hlouis%2fclay/" + tag, function (res) {
      var data = "";
...
function performUpdate(channel) {
  var tag = channel === "beta" ? "beta" : "latest";
  try {
    execFileSync("npm", ["install", "-g", "@hlouis/clay@" + tag], { stdio: "pipe" });
    return true;
  } catch (e) {
    return false;
  }
...
async function checkAndUpdate(currentVersion, skipUpdate) {
  ...
  log(sym.bar + "  " + a.dim + "Run manually: npm install -g @hlouis/clay@latest" + a.reset);
  log("");
  return false;
}
