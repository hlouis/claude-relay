...
var PREVIEW_DENIED_EXTS = new Set([".env", ".pem", ".key", ".crt", ".p12", ".sqlite", ".db"]);
var MAX_PREVIEW_BYTES = 20 * 1024 * 1024; // 20 MB
...
function decodePreviewPath(rawPath) {
  var qIdx = rawPath.indexOf("?");
  var pathPart = qIdx >= 0 ? rawPath.substring(0, qIdx) : rawPath;
  try {
...
function isDeniedPreviewFile(absFile) {
  var base = path.basename(absFile).toLowerCase();
  var ext = path.extname(base).toLowerCase();
  if (PREVIEW_DENIED_EXTS.has(ext)) return true;
...
function attachHTTP(ctx) {
  ...
  function handleHTTP(req, res, urlPath) {
    ...
    // File browser: serve HTML previews and their static assets.
    if (req.method === "GET" && (urlPath === "/api/file-preview" || urlPath.indexOf("/api/file-preview/") === 0)) {
      var previewPrefix = "/api/file-preview/";
      if (urlPath === "/api/file-preview") {
        res.writeHead(400);
        res.end("Missing path");
        return true;
      }
      var previewRelPath = decodePreviewPath(urlPath.substring(previewPrefix.length));
      if (!previewRelPath) {
        res.writeHead(400);
        res.end("Bad path");
        return true;
      }
      var previewAbsFile = safePath(cwd, previewRelPath);
      if (!previewAbsFile) {
        res.writeHead(403);
        res.end("Access denied");
        return true;
      }
      var previewExt = path.extname(previewAbsFile).toLowerCase();
      var previewMime = PREVIEW_MIME_TYPES[previewExt];
      if (!previewMime || isDeniedPreviewFile(previewAbsFile)) {
        res.writeHead(403);
        res.end("Unsupported preview file");
        return true;
      }
      try {
        var previewUserInfo = getOsUserInfoForReq(req);
        var previewStat;
        var previewContent;
        var previewIsDir;
        var previewSize;
        if (previewUserInfo) {
          previewStat = fsAsUser("stat", { file: previewAbsFile }, previewUserInfo);
          previewIsDir = previewStat.isDir;
          previewSize = previewStat.size;
        } else {
          previewStat = fs.statSync(previewAbsFile);
          previewIsDir = previewStat.isDirectory();
          previewSize = previewStat.size;
        }
        if (previewIsDir) {
          res.writeHead(403);
          res.end("Directories cannot be previewed");
          return true;
        }
        if (typeof previewSize === "number" && previewSize > MAX_PREVIEW_BYTES) {
          res.writeHead(413);
          res.end("File too large to preview");
          return true;
        }
        if (previewUserInfo) {
          previewContent = fsAsUser("read_binary", { file: previewAbsFile }, previewUserInfo).buffer;
        } else {
          previewContent = fs.readFileSync(previewAbsFile);
        }
        // The global middleware sets X-Frame-Options: DENY for clickjacking
        // protection; remove it here so the preview can render inside our own
        // sandboxed iframe. The CSP sandbox directive below still prevents
        // direct-navigation XSS, including third-party framing attempts.
        ...
        res.writeHead(200, {
          ...
          // sandbox allow-scripts: scripts run, but the document still has an
          // opaque origin (no allow-same-origin), so it cannot read app
          // cookies/localStorage or reach the parent DOM. allow-top-navigation
          // is intentionally omitted so a malicious page can't escape the
          // iframe.
          "Content-Security-Policy": "sandbox allow-scripts; default-src 'none'; script-src 'self' 'unsafe-inline' 'unsafe-eval' blob:; img-src 'self' data: blob:; style-src 'self' 'unsafe-inline'; font-src 'self' data:; media-src 'self' blob:; connect-src 'self' data: blob:; frame-ancestors 'self'",
        });
    ...
    // File browser: download a file as attachment (any extension within cwd).
    if (req.method === "GET" && urlPath.startsWith("/api/file-download?")) {
      var dlQIdx = urlPath.indexOf("?");
      var dlParams = new URLSearchParams(urlPath.substring(dlQIdx));
      var dlRelPath = dlParams.get("path");
      if (!dlRelPath) { res.writeHead(400); res.end("Missing path"); return true; }
      var dlAbsFile = safePath(cwd, dlRelPath);
      if (!dlAbsFile) { res.writeHead(403); res.end("Access denied"); return true; }
      if (isDeniedPreviewFile(dlAbsFile)) { res.writeHead(403); res.end("File type not allowed"); return true; }
      try {
        var dlUserInfo = getOsUserInfoForReq(req);
        var dlStat;
        var dlIsDir;
        var dlSize;
        if (dlUserInfo) {
          dlStat = fsAsUser("stat", { file: dlAbsFile }, dlUserInfo);
          dlIsDir = dlStat.isDir;
          dlSize = dlStat.size;
        } else {
          dlStat = fs.statSync(dlAbsFile);
          dlIsDir = dlStat.isDirectory();
          dlSize = dlStat.size;
        }
        if (dlIsDir) { res.writeHead(403); res.end("Directories cannot be downloaded"); return true; }
        if (typeof dlSize === "number" && dlSize > MAX_PREVIEW_BYTES) {
          res.writeHead(413); res.end("File too large to download"); return true;
        }
        var dlContent;
        if (dlUserInfo) {
          dlContent = fsAsUser("read_binary", { file: dlAbsFile }, dlUserInfo).buffer;
        } else {
          dlContent = fs.readFileSync(dlAbsFile);
        }
        var dlBase = path.basename(dlAbsFile);
        // RFC 5987 encoded filename for non-ASCII safety
        var dlSafeAscii = dlBase.replace(/[^\x20-\x7E]/g, "_").replace(/"/g, "");
        var dlEncoded = encodeURIComponent(dlBase);
        res.writeHead(200, {
          "Content-Type": "application/octet-stream",
          "Content-Disposition": "attachment; filename=\"" + dlSafeAscii + "\"; filename*=UTF-8''" + dlEncoded,
          "Cache-Control": "no-cache",
          "X-Content-Type-Options": "nosniff",
        });
        res.end(dlContent);
