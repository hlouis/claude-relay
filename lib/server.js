...
function createServer(opts) {
  ...
  // Iter 4: per-project default backend (mutable). Used by the settings
  // panel's backend toggle; affects only NEW sessions thanks to iter 1's
  // immutable session.backend identity.
  var onSetProjectBackend = opts.onSetProjectBackend || null;
  ...
  server.on("upgrade", function (req, socket, head) {
    ...
    wss.handleUpgrade(req, socket, head, function (ws) {
      ...
      clearCrossProjectUnread(wsSlug, ws);
      // Parse resume params from query string for incremental reconnect
      var resumeOpts = null;
      try {
        var qIdx = req.url.indexOf("?");
        if (qIdx !== -1) {
          var params = new URLSearchParams(req.url.substring(qIdx));
          var rSess = params.get("resumeSession");
          var rSeq = params.get("lastSeq");
          if (rSess && rSeq != null) {
            resumeOpts = { sessionId: parseInt(rSess, 10), lastSeq: parseInt(rSeq, 10) };
          }
        }
      } catch (e) {}
      ctx.handleConnection(ws, wsUser, resumeOpts);
    });
  });
  ...
  // Clear cross-project unread for a given project slug when a client views it.
  // Called on new WS connection and on tab_visible (re-focus of existing tab).
  function clearCrossProjectUnread(wsSlug, ws) {
    var unreadMap = getCrossProjectUnread(ws);
    if (unreadMap[wsSlug]) {
      unreadMap[wsSlug] = 0;
    }
    // In single-user mode, clear across ALL clients so badge disappears everywhere
    if (!users.isMultiUser()) {
      var needsBroadcast = false;
      projects.forEach(function (otherCtx, otherSlug) {
        if (otherSlug === wsSlug) return;
        otherCtx.forEachClient(function (otherWs) {
          if (otherWs === ws) return;
          var otherMap = getCrossProjectUnread(otherWs);
          if (otherMap[wsSlug]) {
            otherMap[wsSlug] = 0;
            needsBroadcast = true;
          }
      ...
      if (needsBroadcast) broadcastProcessingChange();
    }
  }
  ...
  // --- Project management ---
  function addProject(cwd, slug, title, icon, projectOwnerId, worktreeMeta, backend, codexConfig, persistCodexConfig) {
    ...
    var ctx = createProjectContext({
      ...
      persistCodexConfig: persistCodexConfig || null,
      ...
      onTabVisible: function (ws) { clearCrossProjectUnread(slug, ws); },
      ...
      onSetProjectBackend: onSetProjectBackend,
