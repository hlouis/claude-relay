...
function formatAskUserAnswerAsMessage(input, answers) {
  ...
  var normalizedAnswers = normalizeAskUserAnswers(input, answers);
  if (questions.length === 0) {
    // Shouldn't happen, but be defensive.
    try { return "(answered with: " + JSON.stringify(normalizedAnswers || {}) + ")"; }
  ...
  for (var i = 0; i < questions.length; i++) {
    ...
    var ans = (normalizedAnswers && normalizedAnswers[qText] != null) ? String(normalizedAnswers[qText]) : "";
    if (!ans) continue;
...
function normalizeAskUserAnswers(input, answers) {
  var source = answers || {};
  var questions = (input && Array.isArray(input.questions)) ? input.questions : [];
  ...
  for (var i = 0; i < questions.length; i++) {
    ...
    var idxKey = String(i);
    var value;
    if (Object.prototype.hasOwnProperty.call(source, qText)) {
      value = source[qText];
      usedKeys[qText] = true;
    } else if (Object.prototype.hasOwnProperty.call(source, idxKey)) {
      value = source[idxKey];
      usedKeys[idxKey] = true;
    }
    if (value != null && String(value).trim()) normalized[qText] = String(value);
  }
  ...
  for (var key in source) {
    ...
    if (source[key] != null && String(source[key]).trim()) normalized[key] = String(source[key]);
  }
  return normalized;
}
...
function attachSessions(ctx) {
  ...
  function buildHistoryDoneMeta(session) {
    var lastUsage = null;
    var lastModelUsage = null;
    var lastCost = null;
    var lastStreamInputTokens = null;
    var total = session && session.history ? session.history.length : 0;
    for (var i = total - 1; i >= 0; i--) {
      if (session.history[i].type === "result") {
        var r = session.history[i];
    ...
    return {
      ...
      contextUsage: session ? (session.lastContextUsage || null) : null,
    };
  }

  function getInitialHistorySlice(session) {
    var total = session.history.length;
    var from = 0;
    if (total > sm.HISTORY_PAGE_SIZE) {
      from = sm.findTurnBoundary(session.history, Math.max(0, total - sm.HISTORY_PAGE_SIZE));
    }
    var items = [];
    for (var i = from; i < total; i++) {
      if (session.history[i] && session.history[i].type === "digest_checkpoint") continue;
      items.push(hydrateImageRefs(session.history[i]));
    }
    return {
      from: from,
      to: total,
  ...
  function visibleHistoryItems(session, from, to) {
    var items = [];
  ...
  function visibleTailFingerprint(session, before) {
    for (var i = before - 1; i >= 0; i--) {
      if (session.history[i] && session.history[i].type === "digest_checkpoint") continue;
      return sm.historyFingerprint ? sm.historyFingerprint(session.history[i]) : "";
    }
    return "";
  }

  function sendHistoryReplace(ws, session) {
    var payload = getInitialHistorySlice(session);
    sendTo(ws, {
      type: "history_replace",
      items: payload.items,
      meta: { from: payload.from, to: payload.to, total: payload.total, hasMore: payload.from > 0 },
      doneMeta: buildHistoryDoneMeta(session),
    });
  }
  ...
  function handleSessionsMessage(ws, msg) {
    ...
    if (msg.type === "load_more_history") {
      ...
      if (msg.initial) {
        sm.replayHistory(session, undefined, ws, hydrateImageRefs);
        return true;
      }
      if (typeof msg.after === "number") {
        var after = msg.after;
        var totalAfter = session.history.length;
        if (after < 0 || after > totalAfter) {
          sendHistoryReplace(ws, session);
          return true;
        }
        if (after > 0 && msg.tailFingerprint) {
          var serverFingerprint = visibleTailFingerprint(session, after);
          if (serverFingerprint !== msg.tailFingerprint) {
            sendHistoryReplace(ws, session);
            return true;
          }
        }
        var appendItems = visibleHistoryItems(session, after, totalAfter);
        sendTo(ws, {
          type: "history_append",
          items: appendItems,
          meta: { from: after, to: totalAfter, total: totalAfter },
          doneMeta: buildHistoryDoneMeta(session),
        });
        return true;
      }
      if (typeof msg.before !== "number") return true;
    ...
    if (msg.type === "ask_user_response") {
      ...
      var normalizedAnswers = normalizeAskUserAnswers(pending.input, answers);
      sm.sendAndRecord(session, { type: "ask_user_answered", toolId: toolId, answers: normalizedAnswers });
      ...
      if (pending.mode === "mcp") {
        ...
        var answerText = formatAskUserAnswerAsMessage(pending.input, normalizedAnswers);
        ...
        pending.resolve({
          behavior: "allow",
          updatedInput: Object.assign({}, pending.input, { answers: normalizedAnswers }),
        });
      }
      return true;
    }
    ...
    if (msg.type === "create_worktree") {
      ...
      var wtExistingBranch = !!msg.existingBranch;
      ...
      if (typeof onCreateWorktree === "function") {
        var wtResult = onCreateWorktree(wtParentSlug, wtBranch, wtDirName, wtBase, wtExistingBranch);
