...
function createCodexBackend(opts) {
  ...
  // When true, an auth_lost emission is in flight: we're about to close the
  // client deliberately so the next Retry re-reads ~/.codex/auth.json from
  // scratch. The onExit handler must NOT replace the auth_lost card with a
  // crashed one in this window. Reset on the next start.
  var gracefulTeardownForAuth = false;
  ...
  function startClientIfNeeded() {
    ...
    client = createCodexClient({
      ...
      onExit: function (code, signal, exitErr) {
        ...
        if (gracefulTeardownForAuth && kind === "crashed") {
          gracefulTeardownForAuth = false;
        } else {
