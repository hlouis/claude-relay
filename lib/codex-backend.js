...
// guidance card.
//
// --- Iter 5 carry-over notes (documented after live verification) ---
//
// `acceptForSession` scope: When the user picks "Always allow" in the
// permission Modal we send Codex the `acceptForSession` decision and ALSO
// cache the allow-key in `session.allowedTools[<allowKey>]`. Both caches
// are deliberately session-scoped:
//   - Codex's own cache lives inside the running app-server process and is
//     wiped on subprocess exit.
//   - Clay's `session.allowedTools` mirror lives in memory only — it does
//     NOT persist to daemon.json or the session JSONL.
// Net effect: a daemon restart, a Codex crash + retry, or a manual logout
// all reset the "always allow" choice. This matches Codex CLI's own
// behavior, but is intentionally weaker than Claude's persistent
// allowedTools list. Documented here instead of guarded by code because
// "weaker scope" is the desired contract; persisting it would diverge
// from upstream Codex semantics.
//
// Network approval channel: Codex routes both shell-command approvals AND
// outbound-network approvals through the SAME RPC method —
// `item/commandExecution/requestApproval`. The distinguishing field is
// `params.networkApprovalContext` (present on network requests, absent on
// pure exec requests). Iter 2's `buildPermissionPayload` forwards the
// network context through `toolInput.network` so the Modal can render it,
// but does NOT route to a separate UI surface. Both kinds count against
// the same `codex:exec` allow-key cache. If a future iteration wants
// distinct "trust this network destination forever" UX it would need a
// new allow-key and a new Modal variant; deliberately deferred.
...
function createCodexBackend(opts) {
  ...
  // Iter 6a: skills cache. Populated by `fetchSkills` after a successful
  // `skills/list` RPC; invalidated when codex pushes a `skills/changed`
  // notification. The cache is a derived view of codex-side state — we do
  // NOT persist it to daemon.json or the session JSONL. A daemon restart
  // re-fetches naturally on warmup. Shape:
  //   { skills: SkillMetadata[], errors: SkillErrorInfo[], fetchedAt: ms }
  // null = never fetched (e.g. warmup failed, or pre-fetch).
  var skillsCache = null;
  // In-flight `skills/list` RPC, deduped so rapid `skills/changed`
  // notifications don't pile up parallel requests.
  var skillsFetchInflight = null;
  ...
  // Iter 5b: build the thread/start params from the desired settings.
  // Factored out because both `ensureThread` (fresh start) and the
  // resume-fallback path use it.
  ...
  // Iter 5b: surface the resolved model on the topbar after a successful
  // thread/start or thread/resume so the UI chip stays in sync. Extracted
  // from the inline thread/start handler.
  function adoptThreadResponse(resp) {
    threadInfo = resp;
    threadId = resp && resp.thread && resp.thread.id;
    if (resp && resp.model) {
  ...
  function startNewThread(session) {
    return client.request("thread/start", buildStartParams()).then(function (resp) {
      adoptThreadResponse(resp);
  ...
  // Iter 5b: resume an existing codex thread (server-side persisted) so
  // sidebar session switching keeps the same threadId on Codex's side and
  // model context carries over. Probe-confirmed: thread/resume returns the
  // same thread.id round-trip and accepts subsequent turn/start.
  function resumeExistingThread(session) {
    var targetId = session.cliSessionId;
    if (!targetId) return Promise.reject(new Error("resume requires cliSessionId"));
    return client.request("thread/resume", { threadId: targetId }).then(function (resp) {
      adoptThreadResponse(resp);
      // Probe asserted thread.id round-trips; defend in case codex changes
      // the contract.
      if (threadId && threadId !== targetId) {
        console.warn("[codex-backend] thread/resume returned a different id (" +
          threadId + " vs " + targetId + "); adopting the new one");
        session.cliSessionId = threadId;
        sm.saveSessionFile(session);
        sendAndRecord(session, { type: "session_id", cliSessionId: threadId });
      }
  ...
  function ensureThread(session) {
    ...
    // Iter 5b: if the session was created by a fork, or restored from
    // disk, or switched-in from the sidebar, it already has a cliSessionId
    // pointing at a server-side codex thread. Resume it instead of
    // starting a new one — that's how multi-thread inside one codex
    // process works (probe step 9-10 verified). Resume failures fall back
    // to a fresh thread so the user is never stuck.
    if (session.cliSessionId) {
      return resumeExistingThread(session).catch(function (e) {
        // The most common reason for resume to fail is a daemon-restart
        // race where codex no longer has the thread (different process,
        // different in-memory state). Surface a soft notice and rebuild.
        console.warn("[codex-backend] thread/resume failed (" +
          (e && (e.message || e)) + "); falling back to thread/start");
        sendAndRecord(session, {
          type: "info",
          text: "Codex could not resume the previous thread; starting a fresh one. Earlier messages remain visible but the model has lost their context.",
        });
        return startNewThread(session);
      });
    }
    return startNewThread(session);
  }

  // Iter 5b: HEAD-only thread fork.
  //
  // Sequence:
  //   1. Ensure the source session's thread is the active one on codex's
  //      side. If sidebar switched but we never sent a turn yet, the
  //      backend's `threadId` may not match — refresh via ensureThread.
  //   2. Call `thread/fork { threadId }` against the source thread.
  //   3. Receive a fresh `Thread { id, forkedFromId }` from codex.
  //   4. Build a new Clay session with cliSessionId = new threadId, copy
  //      source session.history wholesale (Codex already has the same
  //      history server-side; this just makes Clay's UI replay match), and
  //      register it via the session manager so the sidebar updates.
  //
  // What we deliberately do NOT do:
  //   - Send `excludeTurns: true` (would break the "model remembers
  //     ALPHA" UX).
  //   - Pass `atTurnId` / `atItemId` (probe-confirmed silently ignored;
  //     plan kills per-message anchor for 5b).
  //   - Override model/sandbox at fork time (5b inherits current config).
  //
  // Returns a Promise resolving to the NEW session object so the caller
  // (project.js fork_thread handler) can switch the sidebar to it.
  function forkActiveThread(sourceSession) {
    if (!client) return Promise.reject(new Error("Codex client not connected"));
    if (!sourceSession) return Promise.reject(new Error("forkActiveThread requires a source session"));

    return ensureThread(sourceSession).then(function (sourceThreadId) {
      if (!sourceThreadId) throw new Error("source session has no codex threadId");
      return client.request("thread/fork", { threadId: sourceThreadId }).then(function (resp) {
        var newThreadId = resp && resp.thread && resp.thread.id;
        if (!newThreadId || newThreadId === sourceThreadId) {
          throw new Error("thread/fork did not return a new thread id");
        }
        var forkedFromId = resp.thread.forkedFromId || sourceThreadId;

        // Clone source history so the new session's UI replay matches
        // what's on codex's side. Codex's server-side fork already copied
        // its own item store; this is purely for Clay's JSONL.
        var clonedHistory = sourceSession.history
          ? sourceSession.history.slice()
          : [];

        // createSession returns a fresh session object with its own
        // localId. We take ownership of cliSessionId + history before any
        // saveSessionFile call so the JSONL on disk is correct on first
        // write.
        var newSession = sm.createSession({
          ownerId: sourceSession.ownerId || null,
          sessionVisibility: sourceSession.sessionVisibility || "shared",
          // Fork inherits the source session's backend, NOT the project's
          // current default. A Codex thread fork must run on a Codex SDK —
          // the thread id only makes sense to that backend. After iter 4
          // (mutable project.backend) this rule prevents a project that
          // flipped to Claude default from breaking fork of an old Codex
          // session. Today sourceSession.backend is always "codex" here
          // because forkActiveThread is gated on backend === "codex" in
          // project.js, but we pass it explicitly so the rule is enforced
          // at the data layer rather than relying on the upstream guard.
          backend: sourceSession.backend || "codex",
        });
        newSession.cliSessionId = newThreadId;
        newSession.history = clonedHistory;
        newSession.title = (sourceSession.title || "Codex thread") + " (fork)";
        newSession.codexForkedFromId = forkedFromId;
        sm.saveSessionFile(newSession);

        // Switch backend's active state to the fork — the next turn will
        // run against newThreadId. ensureThread short-circuits because
        // threadId now matches.
        threadId = newThreadId;
        currentSession = newSession;
        threadInfo = resp;

        sendAndRecord(newSession, { type: "session_id", cliSessionId: newThreadId });
        if (resp.model) {
          sm.currentModel = resp.model;
          send({ type: "model_info", model: resp.model, models: [], backend: "codex" });
          sendConfigState();
  ...
  // --- Iter 6a: Skills ---
  //
  // `skills/list` returns a cwd-scoped list of available skills. We call
  // it on warmup (best-effort) and on `skills/changed` notifications. The
  // resulting list is broadcast to the frontend as `codex_skills` so the
  // skill picker can render without an extra round-trip per open.
  //
  // Skill invocation is server-side: when the user's outgoing message
  // starts with `$<skill-name> ` we additionally append a `{type:"skill",
  // name, path}` input item to the turn so codex's app-server can inject
  // full skill instructions instead of relying on model-side parsing
  // (per app-server README "Skills" section). See `applySkillInjection`
  // below.
  function fetchSkills(forceReload) {
    if (!client || client.isExited()) {
      return Promise.reject(new Error("Codex client not ready"));
    }
    // Dedupe rapid calls so a burst of skills/changed notifications doesn't
    // queue parallel RPCs.
    ...
    skillsFetchInflight = client.request("skills/list", params).then(function (resp) {
      // Response shape: { data: [{ cwd, skills: [...], errors: [...] }] }
      // We're scoped to one cwd so we flatten to the first entry. Defensive
      // against schema drift (data missing / empty / extra entries).
      var entries = (resp && Array.isArray(resp.data)) ? resp.data : [];
      var match = null;
      for (var i = 0; i < entries.length; i++) {
        if (entries[i] && entries[i].cwd === cwd) { match = entries[i]; break; }
      }
      if (!match && entries.length > 0) match = entries[0];
      var skills = (match && Array.isArray(match.skills)) ? match.skills : [];
      var errors = (match && Array.isArray(match.errors)) ? match.errors : [];
      skillsCache = { skills: skills, errors: errors, fetchedAt: Date.now() };
      send({
        type: "codex_skills",
        skills: skills,
        errors: errors,
        cwd: cwd,
        fetchedAt: skillsCache.fetchedAt,
      });
      return skillsCache;
    }).catch(function (e) {
      // Fetch failures are non-fatal — the picker will simply be empty
      // until the next retry. Method-not-found means the user's codex CLI
      // predates skills/list; we surface a `codex_skills` with an explicit
      // error so the UI can render an "upgrade codex" hint.
      var msg = (e && e.message) ? e.message : String(e);
      if (e && e.code === -32601) {
        send({
          type: "codex_skills",
          skills: [],
          errors: [{ code: "method_not_found", message: "This Codex CLI does not expose skills/list. Run `codex update` to enable Skills." }],
          cwd: cwd,
          fetchedAt: Date.now(),
        });
      } else {
        console.error("[codex-backend] skills/list failed:", msg);
      }
      throw e;
    }).then(function (v) { skillsFetchInflight = null; return v; }, function (e) { skillsFetchInflight = null; throw e; });

    return skillsFetchInflight;
  }

  // Parse `$<skill-name> ...` prefix from outgoing user text. Returns the
  // matching SkillMetadata from the cache, or null if no skill mention
  // is present / cache is empty / name doesn't match.
  //
  // Matching rules (mirrors codex's mention_counts.rs behavior):
  //   - Text must start with `$` followed by a skill name token, then
  //     whitespace OR end-of-string.
  //   - Name token = [^\s$] (stops at first whitespace)
  //   - Match is exact (case-sensitive) against `skill.name`.
  //   - If no exact match, no injection happens — the user sees their `$`
  //     prefix go to codex unmodified (codex's model can still recognize
  //     it, just slower per the README's "omit the skill item" path).
  function findSkillMention(text) {
    if (typeof text !== "string" || text.length === 0 || text.charAt(0) !== "$") return null;
    if (!skillsCache || !Array.isArray(skillsCache.skills) || skillsCache.skills.length === 0) return null;
    var m = /^\$([^\s$]+)(?:\s|$)/.exec(text);
    if (!m) return null;
    var name = m[1];
    for (var i = 0; i < skillsCache.skills.length; i++) {
      var s = skillsCache.skills[i];
      if (s && s.name === name) return s;
    }
    return null;
  }

  // Build the input items array for `turn/start`. When the text has a
  // recognized `$<skill-name>` prefix we append the skill input item so
  // codex injects the full SKILL.md content instead of letting the model
  // resolve it heuristically.
  function applySkillInjection(text) {
    var items = [{ type: "text", text: text || "" }];
    var skill = findSkillMention(text);
    if (skill) {
      // skill.path can be a string or { path: string } depending on how
      // codex serialized AbsolutePathBuf; normalize to a plain string.
      var pathStr = (typeof skill.path === "string")
        ? skill.path
        : (skill.path && typeof skill.path.path === "string" ? skill.path.path : null);
      if (pathStr) {
        items.push({ type: "skill", name: skill.name, path: pathStr });
      }
    }
    return items;
  }
  ...
  function handleNotification(msg) {
    ...
    // Iter 6a: skills file watcher fired. Treat as cache invalidation
    // signal — refetch (cheap; no model call) and broadcast the new list.
    // We do NOT pass forceReload here: codex already invalidated its
    // internal cache before sending the notification.
    if (method === "skills/changed") {
      fetchSkills(false).catch(function () { /* fetchSkills logs */ });
      return;
    }
  ...
  async function startQuery(session, text, images, _linuxUser) {
    ...
    // Iter 6a: parse `$<skill-name>` prefix and append a skill input item
    // when it matches a known skill. Falls through to plain text otherwise.
    var input = applySkillInjection(text);
    try {
  ...
  async function warmup(_linuxUser) {
    ...
    try {
      ...
      // Iter 6a: prefetch skills in the background so the skill picker is
      // populated before the user opens it. Best-effort: a failure just
      // means the picker shows empty until the user clicks Refresh.
      fetchSkills(false).catch(function () { /* fetchSkills already logs */ });
    } catch (e) {
      if (e.codexAuthCode) {
        send({
  ...
  return {
    ...
    // Iter 5b: HEAD-only thread fork. project.js's fork_thread handler
    // calls this with the active session and switches the sidebar to the
    // returned new session.
    forkActiveThread: forkActiveThread,
    // Iter 6a: skills surface. project.js routes user-initiated refresh
    // (`request_codex_skills`) to fetchSkills(true); the cached snapshot
    // is exposed via getSkills so first-connection echo can replay the
    // last known list before re-fetching.
    fetchSkills: fetchSkills,
    getSkills: function () {
      if (!skillsCache) return null;
      return {
        skills: skillsCache.skills.slice(),
        errors: skillsCache.errors.slice(),
        cwd: cwd,
        fetchedAt: skillsCache.fetchedAt,
      };
    },
    ...
    // Iter 5b test hooks: drive ensureThread / fork without spawning a
    // codex subprocess. The threadId field is otherwise only mutated by
    // RPC paths, so tests need a way to seed it.
    _setActiveThreadIdForTest: function (id) { threadId = id; },
    _ensureThreadForTest: function (session) { return ensureThread(session); },
    ...
    // Iter 6a test hooks: seed/inspect the skills cache without RPC, and
    // exercise the `$name` parser directly.
    _setSkillsCacheForTest: function (cache) { skillsCache = cache; },
    _getSkillsCacheForTest: function () { return skillsCache; },
    _findSkillMentionForTest: function (text) { return findSkillMention(text); },
    _applySkillInjectionForTest: function (text) { return applySkillInjection(text); },
    _handleNotification: function (msg) { handleNotification(msg); },
  };
}
