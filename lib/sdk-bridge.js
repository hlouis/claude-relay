...
function createSDKBridge(opts) {
  ...
  // Build and send a config_state message with sensible defaults.
  // Pass overrides to replace individual fields (e.g. { mode: "bypassPermissions" }).
  function sendConfigState(overrides) {
    var msg = {
      type: "config_state",
    ...
    if (overrides) {
      for (var k in overrides) msg[k] = overrides[k];
    }
    send(msg);
  }
  ...
  function processSDKMessage(session, parsed) {
    ...
    if (parsed.type === "system" && parsed.subtype === "init") {
      ...
      // Sync permission mode from SDK init to keep UI in sync with agent state
      if (parsed.permissionMode && !dangerouslySkipPermissions) {
        sm.currentPermissionMode = parsed.permissionMode;
        sendConfigState();
      }
    }
  ...
  async function startQueryViaWorker(session, text, images, linuxUser) {
    ...
    if (sm.currentThinking === "disabled") {
      ...
      queryOptions.thinking = { type: "enabled", budgetTokens: sm.currentThinkingBudget, display: "summarized" };
    } else {
  ...
  async function warmupViaWorker(linuxUser) {
    ...
    worker.onMessage(function(msg) {
      if (msg.type === "warmup_done" && !warmupDone) {
        ...
        send({ type: "model_info", model: sm.currentModel || "", models: sm.availableModels || [] });
        if (result.permissionMode && !dangerouslySkipPermissions) {
          sm.currentPermissionMode = result.permissionMode;
          sendConfigState();
        }
        worker.kill();
    ...
    try {
      ...
      for await (var msg of stream) {
        if (msg.type === "system" && msg.subtype === "init") {
          ...
          send({ type: "model_info", model: sm.currentModel || "", models: sm.availableModels || [] });
          if (msg.permissionMode && !dangerouslySkipPermissions) {
            sm.currentPermissionMode = msg.permissionMode;
            sendConfigState();
          }
          ac.abort();
          break;
  ...
  async function setPermissionMode(session, mode) {
    ...
    if (dangerouslySkipPermissions) {
      sendConfigState({ mode: "bypassPermissions" });
      return;
    }
    if (session.worker) {
...
// Setting keys this backend honors. Surfaced via getBackendCapabilities()
// so the frontend can render only the controls that actually do something.
// Keep this list narrow — every key here implies project.js routes a
// `set_*` message to a real method on the bridge.
var SUPPORTED_SETTINGS = ["model", "permissionMode", "effort", "betas", "thinking"];

module.exports = { createSDKBridge, createMessageQueue, SUPPORTED_SETTINGS: SUPPORTED_SETTINGS };
