...
function findCodexPath() {
  var envPath = process.env.CLAY_CODEX_BIN || process.env.CODEX_BIN || "";
  if (envPath) {
    if (isExecutableFile(envPath)) return envPath;
    throw new Error("Configured codex binary does not exist or is not executable: " + envPath);
  }

  var packageError = null;
  ...
  try {
    ...
    var bundledPath = path.join(vendorRoot, triple, "codex", binaryName);
    if (isExecutableFile(bundledPath)) return bundledPath;
    packageError = new Error("Bundled codex binary is missing or not executable: " + bundledPath);
  } catch (e) {
    packageError = e;
  }

  var pathBinary = findExecutableOnPath("codex");
  if (pathBinary) return pathBinary;

  throw new Error("Could not find codex binary: " + packageError.message);
}
...
function findExecutableOnPath(name) {
  ...
  var dirs = pathValue.split(path.delimiter);
  var extensions = [""];
  if (process.platform === "win32") {
    var pathExt = process.env.PATHEXT || ".EXE;.CMD;.BAT;.COM";
    extensions = pathExt.split(";").map(function(ext) { return ext.toLowerCase(); });
    if (extensions.indexOf("") === -1) extensions.unshift("");
  }

  for (var i = 0; i < dirs.length; i++) {
    if (!dirs[i]) continue;
    for (var j = 0; j < extensions.length; j++) {
      var candidate = path.join(dirs[i], name + extensions[j]);
