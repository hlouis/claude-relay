...
// Parse a model/list RPC response into the cached-models / default-id pair
// used by buildReadyResponse(). Pure function — exported for tests.
//
// Inputs:
//   rpcResult: response from `model/list` (or null/undefined on RPC failure)
//   fallbackModels: array of plain string ids to use if rpcResult is unusable
//   fallbackDefault: string id to use as default in fallback path
//
// Returns: { models, defaultModelId, source: "rpc" | "fallback" }
function parseCodexModelList(rpcResult, fallbackModels, fallbackDefault) {
  if (rpcResult && Array.isArray(rpcResult.data) && rpcResult.data.length > 0) {
    var visible = [];
    var defaultId = null;
    for (var i = 0; i < rpcResult.data.length; i++) {
      var m = rpcResult.data[i];
      ...
      if (m.isDefault && !defaultId) defaultId = m.id;
    }
    if (visible.length > 0) {
      return {
        models: visible,
        defaultModelId: defaultId || visible[0].value,
        source: "rpc",
      };
    }
  }
  return {
    models: (fallbackModels || []).slice(),
    defaultModelId: fallbackDefault || null,
    source: "fallback",
  };
}
...
function createCodexAdapter(opts) {
  ...
  function buildReadyResponse(skillNames) {
    return {
      ...
      defaultModel: _defaultModelId || "gpt-5.5",
      skills: skillNames || [],
  ...
  var adapter = {
    ...
    init: function(initOpts) {
      ...
      _initPromise = (async function() {
        ...
        // Discover models dynamically from the app-server. Older Codex CLI
        // versions don't implement model/list — fall back to a hardcoded
        // list in that case so we never break userspace.
        var FALLBACK_MODELS = [
          "gpt-5.5",
          "gpt-5.4",
          "gpt-5.4-mini",
        ...
        var FALLBACK_DEFAULT = "gpt-5.5";

        var modelListResult = await _appServer.send("model/list", {}, 10000)
          .catch(function(e) {
            console.error("[codex] model/list failed, using fallback list:", e.message);
            return null;
          });

        var parsed = parseCodexModelList(modelListResult, FALLBACK_MODELS, FALLBACK_DEFAULT);
        _cachedModels = parsed.models;
        _defaultModelId = parsed.defaultModelId;
        var modelIds = parsed.models.map(function(x) {
          return typeof x === "string" ? x : x.value;
        });
        console.log("[codex] App-server initialized, models (" + parsed.source + "): " +
          modelIds.join(", ") + " (default: " + _defaultModelId + ")");
    ...
    createQuery: async function(queryOpts) {
      ...
      var model = queryOpts.model || _defaultModelId || "gpt-5.5";
...
module.exports = {
  ...
  // Exported for tests; not part of the public adapter API.
  __test: {
    parseCodexModelList: parseCodexModelList,
  },
};
