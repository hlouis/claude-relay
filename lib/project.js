...
function createProjectContext(opts) {
  ...
  var sdk = createAgentBackend({
    ...
    // Codex-only; ignored by Claude backend.
    codexConfig: initialCodexConfig,
    onCodexConfigChange: function (snapshot) { persistCodexConfig(snapshot); },
  });
  ...
  function handleConnection(ws, wsUser, resumeOpts) {
    ...
    sendTo(ws, { type: "info", cwd: cwd, slug: slug, project: title || project, version: currentVersion, debug: !!debug, dangerouslySkipPermissions: dangerouslySkipPermissions, osUsers: osUsers, lanHost: lanHost, projectCount: _filteredProjects.length, projects: _filteredProjects, projectOwnerId: projectOwnerId, backend: backend, codex: _codexInfo, capabilities: getBackendCapabilities(backend) });
    // Iter 4 follow-up: codex_config first-connection echo. Codex projects
    // expose persisted sandbox/approval/model/effort that the frontend's
    // hard-coded defaults wouldn't reflect; send a snapshot right after
    // info so the chip + settings drawer paint with the correct values on
    // the very first frame.
    if (backend === "codex" && typeof sdk.getCodexConfig === "function") {
      var _codexConfigSnap = sdk.getCodexConfig();
      sendTo(ws, {
        type: "codex_config",
    ...
    // Iter 6a: replay the cached skills list so a reconnecting client
    // doesn't see an empty picker until the next skills/changed or manual
    // refresh. If the cache is null (warmup hasn't completed yet), the
    // upcoming `codex_skills` broadcast from fetchSkills will fill it in.
    if (backend === "codex" && typeof sdk.getSkills === "function") {
      var _skillsSnap = sdk.getSkills();
      if (_skillsSnap) {
        sendTo(ws, {
          type: "codex_skills",
  ...
  function handleMessage(ws, msg) {
    ...
    if (msg.type === "fork_thread") {
      if (backend !== "codex" || typeof sdk.forkActiveThread !== "function") {
        sendTo(ws, { type: "codex_fork_error", reason: "fork is only supported on Codex projects" });
        return;
      }
      ...
      sdk.forkActiveThread(sourceSession).then(function (newSession) {
    ...
    // Iter 3 step 3: Codex-only settings. Routed straight through to the
    // backend, which validates the value and persists it as the next
    // thread/start's parameter. Backends without these methods (Claude)
    // silently ignore.
    if (msg.type === "set_codex_sandbox" && msg.sandbox) {
      if (typeof sdk.setSandbox === "function") {
        var session = getSessionForWs(ws);
        sdk.setSandbox(session, msg.sandbox);
      }
      return;
    }
    if (msg.type === "set_codex_approval_policy" && msg.approvalPolicy) {
      if (typeof sdk.setApprovalPolicy === "function") {
        var session = getSessionForWs(ws);
        sdk.setApprovalPolicy(session, msg.approvalPolicy);
      }
      return;
    }

    // Iter 6a: user-initiated skills refresh. Any client (Codex backend
    // only) can request a forceReload; the resulting `codex_skills`
    // broadcast goes to all connected clients so the picker stays in
    // sync across browser tabs. Backends without fetchSkills no-op.
    if (msg.type === "request_codex_skills") {
      if (typeof sdk.fetchSkills === "function") {
        sdk.fetchSkills(!!msg.forceReload).catch(function () { /* fetchSkills logs */ });
      }
      return;
    }
    ...
    // are no-ops for backends that don't implement them.
    if (msg.type === "codex_retry") {
      if (typeof sdk.retry === "function") sdk.retry();
      return;
    }
    if (msg.type === "codex_logs_request") {
      if (typeof sdk.getLogs === "function" && ws.readyState === 1) {
        var resp;
        try {
          var logs = sdk.getLogs() || {};
          resp = {
  ...
  function setTitle(newTitle) {
    title = newTitle || null;
    send({ type: "info", cwd: cwd, slug: slug, project: title || project, version: currentVersion, debug: !!debug, osUsers: osUsers, lanHost: lanHost, projectCount: getProjectCount(), projects: getProjectList(), projectOwnerId: projectOwnerId, backend: backend, codex: computeCodexAvailability(), capabilities: getBackendCapabilities(backend) });
  }
