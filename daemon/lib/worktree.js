...
// Returns array of { path, branch, bare, detached, accessible, absPath, dirName }
// All worktrees discovered by git are considered accessible regardless of location
function scanWorktrees(projectPath) {
  var resolvedParent = path.resolve(projectPath);
  try {
    // Prune stale worktree entries (directories deleted but git metadata remains)
    try {
    ...
    } catch (e) { /* prune failure is non-fatal */ }
    ...
    for (var i = 0; i < all.length; i++) {
      ...
      wt.accessible = true;
      wt.absPath = resolvedWt;
      wt.dirName = path.basename(wt.path);
      // Stable slug suffix: use branch name with / replaced by -, fall back to dirName
      wt.slugSuffix = wt.branch ? wt.branch.replace(/\//g, "-") : wt.dirName;
      results.push(wt);
    }
    return results;
  } catch (e) {
...
// Create a new worktree as a sibling directory of the parent project
// Directory name: {parentDirName}-{branchName} with / replaced by -
// Returns { ok, path, error }
function createWorktree(projectPath, branchName, baseBranch) {
  var resolvedParent = path.resolve(projectPath);
  var parentDirName = path.basename(resolvedParent);
  var safeBranch = branchName.replace(/\//g, "-");
  var wtDirName = parentDirName + "-" + safeBranch;
  var wtPath = path.resolve(resolvedParent, "..", wtDirName);
...
// Returns { ok, error }
function removeWorktree(projectPath, wtAbsPath) {
  var resolvedParent = path.resolve(projectPath);
  var wtPath = wtAbsPath || resolvedParent;
