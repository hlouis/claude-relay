...
var rewindUserText = null;
var rewindLoadingEl = null;
var rewindLoadingTimeout = null;
...
export function getRewindUserText() {
  return rewindUserText;
}

export function clearRewindUserText() {
  rewindUserText = null;
}

function showRewindLoading() {
  hideRewindLoading();
  rewindLoadingEl = document.createElement("div");
  rewindLoadingEl.className = "rewind-loading-modal";
  rewindLoadingEl.innerHTML =
    '<div class="confirm-backdrop"></div>' +
    '<div class="confirm-dialog rewind-loading-dialog">' +
      '<div class="rewind-loading-content">' +
        '<div class="rewind-loading-spinner"></div>' +
        '<span class="rewind-loading-text">Preparing rewind...</span>' +
      '</div>' +
    '</div>';
  document.body.appendChild(rewindLoadingEl);

  rewindLoadingTimeout = setTimeout(function() {
    hideRewindLoading();
    pendingRewindUuid = null;
    rewindUserText = null;
    ctx.addSystemMessage("Rewind preview timed out.", true);
  }, 30000);
}

export function hideRewindLoading() {
  if (rewindLoadingTimeout) {
    clearTimeout(rewindLoadingTimeout);
    rewindLoadingTimeout = null;
  }
  if (rewindLoadingEl) {
    rewindLoadingEl.remove();
    rewindLoadingEl = null;
  }
}
...
export function initRewind(_ctx) {
  ...
  ctx.messagesEl.addEventListener("click", function(e) {
    ...
    if (msgEl) {
      ...
      if (bubble) rewindUserText = bubble.textContent.trim();
      initiateRewind(msgEl.dataset.uuid);
    }
  });
