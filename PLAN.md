...
| R7 | `protocol/types.ts` doesn't cover every message the daemon emits — e.g. `client_count` is sent on connect but isn't in the Tier 1 schema; and `info.osUsers` is declared `OsUser[]?` while the daemon sends boolean `false` when the feature is disabled (default config), which silently breaks `info` decoding and prevents the `.live` transition against a default daemon | High | M2 dispatcher already drops unknown types with a log line, so unknown messages are safe. The osUsers shape mismatch is **not** safe — must be reconciled before M9 smoke. Before M8, do a one-pass audit of `daemon/lib/project.js` `sendTo(...)` call sites and reconcile with `protocol/types.ts` and the Apple Codable mirror. Either widen the schema (boolean \| array) or align the daemon. Add missing Tier 1 messages, defer the rest to Tier 2 explicitly. |
